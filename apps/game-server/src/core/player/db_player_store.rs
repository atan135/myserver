...
const PLAYER_SCHEMA_STATEMENTS: &[&str] = &[
    ...
    "CREATE INDEX IF NOT EXISTS idx_player_inventory_updated_at ON player_inventory (updated_at)",
    ...
    "CREATE INDEX IF NOT EXISTS idx_player_inventory_grants_player_id ON player_inventory_grants (player_id)",
];

/// PostgreSQL Player Inventory Store.
/// 负责玩家背包数据的持久化。
#[derive(Clone)]
...
impl PgPlayerStore {
    ...
    pub async fn save(&self, player_id: &str, data: &PlayerData) -> Result<(), String> {
        ...
        sqlx::query(UPSERT_PLAYER_INVENTORY_SQL)
            .bind(player_id)
            .bind(data.level)
            .bind(data.get_hp())
            .bind(json.inventory)
    ...
    pub async fn load(&self, player_id: &str) -> Result<Option<PlayerData>, String> {
        ...
        let row = sqlx::query_as::<_, PlayerInventoryRow>(
            r#"SELECT
                level,
                hp,
                inventory_data,
        ...
        row.map(|row| row.into_player_data(player_id)).transpose()
    }
    ...
    pub async fn delete(&self, player_id: &str) -> Result<(), String> {
        ...
        sqlx::query(r#"DELETE FROM player_inventory WHERE player_id = $1"#)
            .bind(player_id)
            .execute(pool)
            .await
            .map_err(|e| e.to_string())?;
...
const UPSERT_PLAYER_INVENTORY_SQL: &str = r#"INSERT INTO player_inventory (
    player_id,
    level,
    hp,
    inventory_data,
...
impl PlayerInventoryRow {
    fn into_player_data(self, player_id: &str) -> Result<PlayerData, String> {
...
async fn initialize_schema(pool: &PgPool) -> Result<(), sqlx::Error> {
    ...
    for statement in PLAYER_SCHEMA_STATEMENTS {
