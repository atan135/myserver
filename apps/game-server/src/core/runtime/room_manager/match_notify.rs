...
impl RoomManager {
    pub(super) async fn notify_room_created(
        ...
        if let Some(ref mut client) = *guard {
            match client
                .create_room_and_join(match_id, room_id, character_ids, mode)
                .await
            {
                Ok(()) => {
                    info!(
    ...
    pub(super) async fn notify_player_joined(
        ...
        let mut guard = self.match_client.lock().await;
        if let Some(ref mut client) = *guard {
            match client.player_joined(match_id, character_id, room_id).await {
                Ok(()) => {
                    info!(
                        match_id = match_id,
                ...
                Err(e) => {
                    tracing::error!(match_id = match_id, character_id = character_id, error = %e, "Failed to notify MatchService: player joined");
                }
            }
        }
    }
    ...
    pub(super) async fn notify_player_left(
        ...
        let mut guard = self.match_client.lock().await;
        if let Some(ref mut client) = *guard {
            match client.player_left(match_id, character_id, reason).await {
                Ok(should_abort) => {
                    info!(
                        match_id = match_id,
                        character_id = character_id,
                ...
                Err(e) => {
                    tracing::error!(match_id = match_id, character_id = character_id, error = %e, "Failed to notify MatchService: player left");
                }
            }
        }
        false
