...
pub const SKILLBASE_SCHEMA_SIGNATURE: &str = "Id:int|Code:string|Name:string|CooldownFrames:int|CastFrames:int|Range:float|TargetType:string|EffectScript:string|Description:string";

#[derive(Debug, Clone, Default)]
pub struct SkillBaseRow {
    pub id: i32,
    pub code: StringKey,
    pub name: StringKey,
    pub cooldownframes: i32,
    pub castframes: i32,
    pub range: f32,
    pub targettype: StringKey,
...
pub struct SkillBase {
    ...
    pub rows: Vec<SkillBaseRow>,
    pub by_id: std::collections::HashMap<i32, usize>,
}

impl CsvTableLoader for SkillBase {
    const TABLE_NAME: &'static str = "SkillBase";
    const SCHEMA_SIGNATURE: &'static str = SKILLBASE_SCHEMA_SIGNATURE;
    ...
    fn load_from_csv(path: &std::path::Path) -> Result<Self, CsvLoadError> {
        ...
        for (row_offset, line) in lines.enumerate() {
            ...
            let row = SkillBaseRow {
                ...
                cooldownframes: reader.parse_i32(3, "CooldownFrames")?,
                castframes: reader.parse_i32(4, "CastFrames")?,
                range: reader.parse_f32(5, "Range")?,
                targettype: reader.parse_string_key(6, "TargetType", &mut string_pool)?,
                effectscript: reader.parse_string_key(7, "EffectScript", &mut string_pool)?,
                description: reader.parse_string_key(8, "Description", &mut string_pool)?,
            };
...
impl SkillBase {
    pub fn get(&self, id: i32) -> Option<&SkillBaseRow> {
        self.by_id
            .get(&id)
            .and_then(|&row_index| self.rows.get(row_index))
    }

    pub fn all(&self) -> &[SkillBaseRow] {
        &self.rows
    }
