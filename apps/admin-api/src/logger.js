...
const SENSITIVE_TEXT_PATTERN = /(?:GAME_ADMIN_TOKEN|GAME_PROXY_ADMIN_TOKEN|JWT_SECRET|INTERNAL_API_TOKEN|\b(?:authorization|ticket|token|secret|password|content|body|attachments?|payload)\b)/i;
...
export function log(level, category, meta = {}) {
  if (!logger) return;
  const message = formatLogPayload(`[${category}]`, meta);
