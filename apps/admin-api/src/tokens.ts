export const ADMIN_CONFIG = Symbol("ADMIN_CONFIG");
export const ADMIN_REDIS = Symbol("ADMIN_REDIS");
export const ADMIN_NATS = Symbol("ADMIN_NATS");
export const ADMIN_DB_POOL = Symbol("ADMIN_DB_POOL");
export const ADMIN_GAME_DB_POOL = Symbol("ADMIN_GAME_DB_POOL");
export const ADMIN_STORE = Symbol("ADMIN_STORE");
export const ADMIN_POLICY = Symbol("ADMIN_POLICY");
export const ADMIN_SESSION_STORE = Symbol("ADMIN_SESSION_STORE");
export const ADMIN_GAME_ADMIN_CLIENT = Symbol("ADMIN_GAME_ADMIN_CLIENT");
export const ADMIN_AUTH_HTTP_CLIENT = Symbol("ADMIN_AUTH_HTTP_CLIENT");
export const ADMIN_OPERATION_ASSERTIONS = Symbol("ADMIN_OPERATION_ASSERTIONS");
export const ADMIN_OPERATIONS = Symbol("ADMIN_OPERATIONS");
export const ADMIN_BREAKGLASS = Symbol("ADMIN_BREAKGLASS");
export const ADMIN_HIGH_RISK_OPERATIONS = Symbol("ADMIN_HIGH_RISK_OPERATIONS");
export const ADMIN_OPERATION_SAFETY = Symbol("ADMIN_OPERATION_SAFETY");
export const ADMIN_METRICS = Symbol("ADMIN_METRICS");
export const ADMIN_METRICS_ARCHIVE_SCHEDULER = Symbol("ADMIN_METRICS_ARCHIVE_SCHEDULER");
export const ADMIN_REGISTRY = Symbol("ADMIN_REGISTRY");
export const MYFORGE_STORE = Symbol("MYFORGE_STORE");
export const MYFORGE_GATEWAY = Symbol("MYFORGE_GATEWAY");
export const MYFORGE_ORCHESTRATOR = Symbol("MYFORGE_ORCHESTRATOR");
export const ADMIN_ACTIVITY_CONTROL = Symbol("ADMIN_ACTIVITY_CONTROL");
