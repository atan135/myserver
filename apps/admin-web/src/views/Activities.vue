    ...
    <el-drawer v-model="drawer.open" :title="drawer.title" size="min(720px, 92vw)" destroy-on-close :before-close="guardDrawerClose">
      ...
      <template v-else-if="drawer.detail">
        ...
        <el-form ref="draftForm" :model="draft" label-width="100px" class="draft-form">
          ...
          <el-form-item label="标题"><el-input v-model="draft.publicConfig.title" /></el-form-item>
          ...
          <el-form-item label="领取方式"><el-select v-model="draft.publicConfig.claimMode" clearable><el-option label="手动" value="manual" /><el-option label="自动" value="automatic" /></el-select></el-form-item>
          </template>
          <el-form-item label="状态"><el-tag :type="statusTag(drawer.detail.status)">{{ statusLabel(drawer.detail.status) }}</el-tag></el-form-item>
          <el-form-item v-if="isDraft" label="修改原因" required><el-input v-model="reason" type="textarea" :rows="2" /></el-form-item>
        </el-form>
        <template v-if="isDraft">
...
import ActivitySnapshotViewer from "../modules/activity/ActivitySnapshotViewer.vue";
import { activityApi } from "../api";
import { activityError, buildVersionCommand, draftIsDirty, filterActivities, normalizeActivityListResponse } from "../api/activity.js";
...
import { formatHighRiskPreview, normalizeHighRiskError, runHighRiskOperation } from "../operations/high-risk.js";
...
const drawer = reactive({ open: false, title: "活动详情", loading: false, saving: false, publishing: false, offlining: false, forking: false, detail: null, error: null });
...
const canFork = computed(() => ["published", "offline"].includes(drawer.detail?.status));
const isDirty = computed(() => isDraft.value && draftIsDirty(draft, savedDraftSnapshot.value));
...
async function forkDraft() { if (!drawer.detail || !canFork.value) return; try { const prompt = await ElMessageBox.prompt("请输入创建新草稿的原因", "创建新草稿", { inputPattern: /\S+/, inputErrorMessage: "原因不能为空", inputValue: "调整活动配置" }); drawer.forking = true; const response = await activityApi.createDraftFromPublished(drawer.detail.activityId, { sourceVersion: drawer.detail.version, ifMatch: drawer.detail.etag, reason: prompt.value, overrides: {} }); const detail = response.data || response; drawer.detail = detail; Object.keys(draft).forEach((key) => delete draft[key]); Object.assign(draft, detail.draft || {}); draft.publicConfig = { ...(draft.publicConfig || {}) }; resourcesText.value = JSON.stringify(draft.publicConfig.resources || []); savedDraftSnapshot.value = JSON.stringify(draft); reason.value = ""; await loadEditorForType(detail.activityType); await search(); ElMessage.success("新草稿已创建"); } catch (error) { if (error !== "cancel" && error !== "close") { const normalized = activityError(error, "创建新草稿失败"); drawer.error = normalized; ElMessage.error(normalized.message); } } finally { drawer.forking = false; } }
...
async function publish(row) { const detail = row.activityId === drawer.detail?.activityId ? drawer.detail : row; if (isDirty.value) { try { await ElMessageBox.confirm("当前草稿有未保存修改，仍要发布服务器上的版本吗？", "未保存变更", { type: "warning" }); } catch { return; } } const command = buildVersionCommand(detail, reason.value || "发布活动"); if (!command.reason) return ElMessage.warning("请填写发布原因"); try { drawer.publishing = true; const outcome = await runHighRiskOperation({ invoke: (payload) => activityApi.publish(detail.activityId, payload), payload: command, confirm: (preflight) => ElMessageBox.confirm(`${formatHighRiskPreview(preflight)}\n\n确认发布 ${detail.key} v${detail.version}？发布后该版本不可编辑。`, "发布确认", { type: "warning" }).then(() => true).catch(() => false) }); if (outcome.phase === "cancelled") return; if (outcome.phase === "execution_uncertain") { ElMessage.warning("发布结果待核实，请查看操作审计后再处理"); return; } if (outcome.phase !== "succeeded") throw new Error("ACTIVITY_PUBLISH_NOT_COMPLETED"); ElMessage.success("活动已发布"); drawer.detail = outcome.response?.data || outcome.response; await search(); } catch (error) { const normalized = error?.kind ? error : normalizeHighRiskError(error); drawer.error = normalized; ElMessage.error(normalized.serverMessage || normalized.description || activityError(error, "发布活动失败").message); } finally { drawer.publishing = false; } }
async function takeOffline(row) { const detail = row.activityId === drawer.detail?.activityId ? drawer.detail : row; try { const prompt = await ElMessageBox.prompt("请输入下线原因", "下线确认", { inputPattern: /\\S+/, inputErrorMessage: "下线原因不能为空", inputValue: reason.value }); drawer.offlining = true; const outcome = await runHighRiskOperation({ invoke: (payload) => activityApi.offline(detail.activityId, payload), payload: buildVersionCommand(detail, prompt.value), confirm: (preflight) => ElMessageBox.confirm(`${formatHighRiskPreview(preflight)}\n\n确认下线 ${detail.key} v${detail.version}？`, "下线确认", { type: "warning" }).then(() => true).catch(() => false) }); if (outcome.phase === "cancelled") return; if (outcome.phase === "execution_uncertain") { ElMessage.warning("下线结果待核实，请查看操作审计后再处理"); return; } if (outcome.phase !== "succeeded") throw new Error("ACTIVITY_OFFLINE_NOT_COMPLETED"); ElMessage.success("活动已下线"); drawer.detail = outcome.response?.data || outcome.response; await search(); } catch (error) { if (error !== "cancel" && error !== "close") { const normalized = error?.kind ? error : normalizeHighRiskError(error); drawer.error = normalized; ElMessage.error(normalized.serverMessage || normalized.description || activityError(error, "活动下线失败").message); } } finally { drawer.offlining = false; } }
onMounted(search);
</script>

<style scoped>
.activity-page { min-width: 0; } .page-heading { display: flex; justify-content: space-between; align-items: flex-start; gap: 16px; margin-bottom: 16px; } h3 { font-size: 20px; } .muted { color: #909399; margin-top: 6px; } .filters { margin-bottom: 16px; } .pagination { margin-top: 16px; justify-content: flex-end; } .summary { margin-bottom: 18px; } .draft-form { max-width: 680px; } .drawer-actions { display: flex; flex-wrap: wrap; gap: 10px; justify-content: flex-end; margin-top: 18px; } .editor-section, .preflight-panel, .records-panel { margin-top: 18px; min-width: 0; } .subheading { display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px; } .preflight-errors { display: grid; gap: 8px; margin-top: 10px; } .preflight-error { display: grid; grid-template-columns: minmax(180px, 1fr) auto minmax(160px, 2fr) minmax(180px, 2fr); gap: 8px; align-items: center; padding: 8px; } .suggestion { color: var(--el-color-warning); overflow-wrap: anywhere; } .records-filters { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 8px; } .records-filters :deep(.el-form-item) { margin-right: 0; margin-bottom: 0; } .records-filters :deep(.el-input) { width: 140px; } .records-filters :deep(.el-input-number) { width: 120px; } code { white-space: pre-wrap; word-break: break-word; overflow-wrap: anywhere; } @media (max-width: 700px) { .page-heading { align-items: stretch; flex-direction: column; } .page-heading .el-button { align-self: flex-start; } .filters :deep(.el-form-item) { width: 100%; margin-right: 0; } .filters :deep(.el-select), .filters :deep(.el-input), .filters :deep(.el-date-editor) { width: 100%; } .preflight-error { grid-template-columns: 1fr; } .records-filters :deep(.el-form-item), .records-filters :deep(.el-input), .records-filters :deep(.el-input-number) { width: 100%; } }
</style>
