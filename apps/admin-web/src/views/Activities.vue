  ...
  <AdminLayout>
    <section class="activity-page">
      ...
      <el-form class="filters" :inline="true" @submit.prevent="search">
        ...
        <el-form-item label="类型"><el-select v-model="filters.activityType" clearable placeholder="全部" @change="search"><el-option label="登录奖励" value="login_reward" /><el-option label="随机抽奖" value="lottery" /></el-select></el-form-item>
    ...
    <el-drawer v-model="drawer.open" :title="drawer.title" size="min(720px, 92vw)" destroy-on-close :before-close="guardDrawerClose">
      ...
      <template v-else-if="drawer.detail">
        ...
        <div class="drawer-actions"><el-button :disabled="!isDraft" :loading="drawer.saving" @click="saveDraft">保存草稿</el-button><el-button v-if="isDraft" type="success" :loading="drawer.publishing" @click="publish(drawer.detail)">发布</el-button><el-button v-if="drawer.detail.status === 'published'" type="warning" :loading="drawer.offlining" @click="takeOffline(drawer.detail)">下线</el-button></div>
      </template>
    </el-drawer>

    <el-dialog v-model="create.open" title="新建活动" width="min(620px, 92vw)">
      <el-form :model="create.form" label-width="100px"><el-form-item label="活动标识" required><el-input v-model="create.form.key" /></el-form-item><el-form-item label="活动类型" required><el-select v-model="create.form.activityType"><el-option label="登录奖励" value="login_reward" /><el-option label="随机抽奖" value="lottery" /></el-select></el-form-item><el-form-item label="修改原因" required><el-input v-model="create.form.reason" type="textarea" /></el-form-item></el-form>
...
const draft = reactive({}); const savedDraftSnapshot = ref(""); const reason = ref(""); const create = reactive({ open: false, loading: false, form: { key: "", activityType: "login_reward", reason: "" } });
...
function typeLabel(value) { return value === "lottery" ? "随机抽奖" : value === "login_reward" ? "登录奖励" : value || "-"; }
...
function openCreate() { Object.assign(create.form, { key: "", activityType: "login_reward", reason: "" }); create.open = true; }
async function createDraft() { if (!create.form.key.trim() || !create.form.reason.trim()) return ElMessage.warning("请填写活动标识和修改原因"); create.loading = true; try { const response = await activityApi.createDraft({ key: create.form.key.trim(), activityType: create.form.activityType, schemaVersion: 1, startAt: new Date().toISOString(), endAt: new Date(Date.now() + 86400000).toISOString(), claimDeadline: new Date(Date.now() + 172800000).toISOString(), timezone: "UTC", publicConfig: { title: create.form.key.trim() }, typeConfig: create.form.activityType === "lottery" ? { schema_version: 1, draw_source: "player_action", pool_version: 1, free_draw_count: 0, daily_draw_limit: 1, total_draw_limit: 1, pool_items: [{ item_id: 1001, quantity: 1, weight: 1 }] } : { schema_version: 1, event_source: "game_entry", cycle_unit: "natural_day", progression: "consecutive", miss_policy: "reset", claim_mode: "manual", stages: [{ stage_no: 1, required_count: 1, reward_group_key: "default" }] }, stages: create.form.activityType === "lottery" ? [] : [{ stageId: "stage-1", stageNo: 1, rewardGroupKey: "default", qualification: {} }], rewardGroups: [{ key: "default", selectionMode: "fixed", items: create.form.activityType === "lottery" ? [{ item_id: 1001, quantity: 1 }] : [{ quantity: 1 }] }], reason: create.form.reason.trim() }); create.open = false; ElMessage.success("草稿已创建"); await openDetail(response.data || response); await search(); } catch (error) { ElMessage.error(activityError(error, "创建草稿失败").message); } finally { create.loading = false; } }
async function openDetail(row) { if (drawer.open && isDirty.value) { try { await ElMessageBox.confirm("当前草稿有未保存修改，确定切换活动并放弃吗？", "未保存变更", { type: "warning" }); } catch { return; } } drawer.open = true; drawer.loading = true; drawer.error = null; try { const response = await activityApi.detail(row.activityId); const detail = response.data || response; drawer.detail = detail; Object.keys(draft).forEach((key) => delete draft[key]); Object.assign(draft, detail.draft || {}); savedDraftSnapshot.value = JSON.stringify(draft); reason.value = ""; } catch (error) { drawer.error = activityError(error, "获取活动详情失败"); } finally { drawer.loading = false; } }
async function saveDraft() { if (!reason.value.trim()) return ElMessage.warning("请填写修改原因"); drawer.saving = true; try { const response = await activityApi.updateDraft(drawer.detail.activityId, { ...draft, reason: reason.value.trim(), ifMatch: drawer.detail.etag }); drawer.detail = response.data || response; Object.assign(draft, drawer.detail.draft || {}); savedDraftSnapshot.value = JSON.stringify(draft); reason.value = ""; ElMessage.success("草稿已保存"); await search(); } catch (error) { drawer.error = activityError(error, "保存草稿失败"); ElMessage.error(drawer.error.message); } finally { drawer.saving = false; } }
...
<style scoped>
.activity-page { min-width: 0; } .page-heading { display: flex; justify-content: space-between; align-items: flex-start; gap: 16px; margin-bottom: 16px; } h3 { font-size: 20px; } .muted { color: #909399; margin-top: 6px; } .filters { margin-bottom: 16px; } .pagination { margin-top: 16px; justify-content: flex-end; } .summary { margin-bottom: 18px; } .draft-form { max-width: 680px; } .drawer-actions { display: flex; gap: 10px; justify-content: flex-end; margin-top: 18px; } code { white-space: pre-wrap; word-break: break-word; } @media (max-width: 700px) { .page-heading { align-items: stretch; flex-direction: column; } .page-heading .el-button { align-self: flex-start; } .filters :deep(.el-form-item) { width: 100%; margin-right: 0; } .filters :deep(.el-select), .filters :deep(.el-input), .filters :deep(.el-date-editor) { width: 100%; } }
</style>
