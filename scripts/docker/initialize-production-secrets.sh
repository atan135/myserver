...
write_secret_file game-server.env \
  ...
  'GLOBAL_ID_WORKER_LEASE_WAIT_TIMEOUT_SECS=120' \
  'GLOBAL_ID_WORKER_LEASE_RETRY_INITIAL_MS=250' \
  'GLOBAL_ID_WORKER_LEASE_RETRY_MAX_MS=5000' \
  'GAME_SOCKET_ROOT=/run/myserver' \
  'GAME_SOCKET_BASENAME=game-server' \
  'GAME_SOCKET_RECLAIM_TIMEOUT_MS=2000' \
  'GAME_SOCKET_RECLAIM_SPIN_MS=25' \
  'GAME_SOCKET_PROBE_TIMEOUT_MS=100' \
  'GAME_DRAIN_SHUTDOWN_TIMEOUT_SECS=300' \
  'GAME_DRAIN_SHUTDOWN_POLL_MS=250' \
