...
- `game-proxy` 同时注册自身 `client` / `admin` endpoint，并消费注册中心发现 `game-server` upstream。
...
- `auth-http` 只做消费者：`REGISTRY_ENABLED=true` 时发现 `chat-server` / `mail-service` / `announce-service` 并写入登录响应的 `services` 对象；`game` 地址在本地开发可来自静态 `gameProxyHost/gameProxyPort`，测试/线上应按入口服务发布口径或注册中心发现结果下发，避免消费者硬编码内部 host/port。
...
| game-server (admin) | 本地默认 7500 | TCP | 测试/线上通过 Redis 注册中心发现 endpoint；固定端口仅作本地开发/手工 fallback |
...
│   services.chat / services.announce / services.mail         │
│     → 使用 auth-http 下发的 endpoint，不硬编码 900x         │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
                    ┌─────────────────┐
...
下面响应体是本地开发示例；测试/线上应由 `auth-http` 按入口服务发布口径或注册中心发现结果下发 `services` endpoint，消费者不应硬编码示例中的 host/port。
...
2. 附件领取：mail-service 通过 registry 发现 game-server admin endpoint

客户端调用 POST /api/v1/mails/:mailId/claim
       ↓
mail-service 校验归属、过期时间、附件格式
       ↓
mail-service -> service registry -> game-server admin endpoint
       ↓
game-server 发放道具并落库
       ↓
...
- `mail-service` 在附件领取链路上依赖 `game-server admin` 能力；测试/线上应通过 service registry 发现 endpoint，本地开发才可使用固定端口手工 fallback
...
│  auth-http:3000 (登录)                                           │
│  game-proxy:4000 (游戏)                                          │
│  services.chat / services.mail (登录响应下发 endpoint)            │
└─────────────────────────────────────────────────────────────────┘
```
...
下面的 `127.0.0.1` 和端口仅表示本地开发注册样例；测试/线上消费者应读取 registry 记录中的实际 endpoint，不应把这些示例值写入配置。
...
UPSTREAM_LOCAL_SOCKET_NAME=myserver-game-server-001.sock  # 本地开发兼容字段；测试/线上消费者从 registry metadata 发现
```
...
本节请求/响应中的 host/port 仅为本地示例，不能作为测试/线上直连配置。
...
1. 移除测试/线上对静态 `UPSTREAM_LOCAL_SOCKET_NAME` 配置的依赖
...
3. **`game-proxy` 已接入基于注册中心的动态发现**
   ...
   - 发现结果会同步到内部路由表
   - `REGISTRY_ENABLED=false` 时仍保留静态 `UPSTREAM_LOCAL_SOCKET_NAME` 兼容路径，仅用于本地开发/手工 fallback
   - 同时注册 `game-proxy` 自身的 `client`、`client-tcp-fallback` 和 `admin` endpoint，供登录返回、控制面聚合和 rollout 演练发现
   - 代码位置：
...
4. **`match-service` 已消费注册中心中的 `game-server` 内部地址**
   ...
   - 本地开发中若注册中心不可用或未返回 metadata，可回退到 `GAME_SERVER_INTERNAL_SOCKET_NAME`；测试/线上应修复 registry 数据，不依赖该静态配置
   - 代码位置：
...
| game-server(admin) | 本地默认 7500 | 已接入发现口径 | 测试/线上通过注册中心发现 endpoint；固定端口仅用于本地开发/手工 fallback |
| game-proxy | 4000 | 已接入注册与发现 | 注册自身 `client` / `admin` endpoint，并消费注册中心发现 `game-server` |
...
| auth-http | 3000 | 已接入部分发现 | `REGISTRY_ENABLED=true` 时会返回统一 `services`，并从注册中心发现 `chat` / `mail` / `announce`；`gameProxyHost/gameProxyPort` 是旧兼容字段，测试/线上消费者应读取 `services.game` |
...
- `scripts/dev-announce.ps1` - 启动 announce-service
  ...
  - 会默认设置 `DB_ENABLED=false`，便于先以内存存储联调
  - 默认按端口生成 `SERVICE_INSTANCE_ID`
...
- `scripts/dev-auth.ps1` - 启动 auth-http
  ...
  说明：
  - 会自动设置 `REGISTRY_ENABLED=true`
  - 登录 / 签票响应会返回统一 `services` 对象，并保留旧的 `gameProxyHost/gameProxyPort` 字段用于兼容旧客户端
...
下面响应体是本地开发示例；测试/线上新客户端应以 `services` 对象中的实际 endpoint 为准。
...
- `auth-http` 仍保留静态配置下发 `game-proxy` 地址的兼容路径；测试/线上客户端不应直接消费旧字段作为内部直连配置，应优先使用 `services.game`
- 已返回统一的 `services` 对象
...
5. 调用 `auth-http` 登录接口，确认当前会返回统一 `services` 对象，并保留 `gameProxyHost/gameProxyPort` 兼容字段；新客户端验证应以 `services.game` 为准。
...
3. `auth-http` 已接入部分发现，当前会返回统一 `services`，并保留旧 `gameProxyHost/gameProxyPort` 兼容路径；测试/线上消费者应以 `services` 为准。
