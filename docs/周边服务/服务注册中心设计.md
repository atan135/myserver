...
| 客户端硬编码 | 外部 `mybevy` 客户端应消费登录响应中的 `services` / `gameProxyHost` / `gameProxyPort`，不应硬编码直连 `game-server:7000` |
...
当前仓库已经落地 Redis 版服务注册中心，但实现形态和早期设计有几处边界需要明确：
...
- `game-proxy` 可选 Redis route store 持久化会复用 Redis 连接配置，但它保存的是 `proxy:route-store:state` JSON 快照，并通过 `proxy:route-store:updates` pub/sub 频道通知其它 proxy reload；这些 key/channel 不属于服务注册中心的 `service:*:instances:*` 或 `heartbeat:*` schema。
...
├── announce-service/  # 注册 announce-service 服务
