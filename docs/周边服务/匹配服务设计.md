...
Match-Service 是独立的内网匹配服务，负责玩家匹配逻辑；通过 gRPC 为游戏服和内部调用方提供匹配接口并接收 `game-server` 回调，正式客户端不直接发现或直连该服务。创建 matched room 时，Match-Service 通过内部 local socket + Protobuf 请求 `game-server`。
...
Match-Service 当前已接入统一 Redis service registry，服务间调用不再以固定 host/port 或固定 socket 作为测试、预发、线上主路径。

- `match-service` 启动时注册 schema v2 endpoint：`match-service.grpc`，`protocol=grpc`，`visibility=internal`。endpoint 和实例 metadata 包含 `service_name`、`service_instance_id` / `instance_id`、`protocol=grpc`、已配置 match modes、`runtime_store` / `runtime_store_backend`、`build_version`、`zone` 等当前实现信息。
- `DISCOVERY_REQUIRED=true`，或 `APP_ENV` / `NODE_ENV` 为 `test`、`testing`、`staging`、`stage`、`prod`、`production` 时，registry client 创建失败、注册失败或发现失败都应明确失败；测试、预发、线上不允许靠本地 fallback 跑通主链路。
- registry heartbeat 来自 service registry 的 `heartbeat:<service>:<instance>` 续约；`metrics:heartbeat:match-service` 只表示业务 metrics 新鲜度，不能替代 registry 服务发现、endpoint 发布或 registry heartbeat。

双向发现规则：

- `game-server -> match-service`：`game-server` 的 `MatchClient` 发现 `match-service.grpc` endpoint，并将发现到的 `host:port` 组装为 tonic gRPC channel。strict / test / prod 环境必须通过 registry；`MATCH_SERVICE_ADDR` 仅允许在 `development` / `local` 且 discovery 非 required 时作为本地调试 fallback。`MatchClient` 已有 gRPC 失败重连标记和周期 rediscovery，避免 `match-service` 滚动重启或 endpoint 切换后长期绑定旧地址。
- `match-service -> game-server`：`GameServerClient` 发现 `game-server.internal` endpoint，要求 `protocol=local_socket`，`visibility` 只接受 `local` 或 `internal`，并使用共享 discovery cache。候选选择会基于当前 registry 候选集合，按 mode、zone、weight、cache TTL 与稳定 hash 策略做保守选择；没有合格候选时建房失败。`GAME_SERVER_INTERNAL_SOCKET_NAME` / `GAME_INTERNAL_SOCKET_NAME` 仅允许在 `development` / `local` 且 discovery 非 required 时作为本地 fallback；strict registry disabled 或发现不到 endpoint 时不会静默走默认 socket。
...
```
1. 内部调用方 → MatchService.MatchStart
...
6. MatchService → 调用方 (stream) MatchEvent { matched, room_id, token }
7. 调用方根据 room_id 进入房间；当前 token 已生成并回传，但尚未接入 `game-server` 的入房校验
...
```
1. 内部调用方 → MatchService.MatchCancel
2. MatchService 从匹配池移除候选人
3. 如果已经 matched 但没人进入，通知 GameServer abort
4. MatchService → 调用方 MatchEvent { match_cancelled }
```
...
struct Config {
    ...
    game_server_discovery_cache_ttl_secs: u64,
    game_server_target_zone: String,
    ...
    match_runtime_store: String,
    match_runtime_key_prefix: String,
    match_runtime_lease_ttl_secs: u64,
    match_recovery_enabled: bool,
}
...
- `match-service` 已注册 `match-service.grpc` endpoint，metadata 暴露 match modes、runtime store、build version、zone 等发现侧需要的当前实现信息；严格环境下 registry client 创建或注册失败会 fail-fast
- GameServer 侧 `MatchClient` 已通过 registry 发现 `match-service.grpc`，并带周期 rediscovery / reconnect 机制；`MATCH_SERVICE_ADDR` 只保留为 local / development 非严格场景的本地 fallback
- `GameServerClient` 已实现，通过 registry 发现 `game-server.internal` 的 `local_socket` endpoint 连接 `game-server` 内部通道发起 `CreateMatchedRoomReq`；候选 endpoint 必须健康，且 `visibility` 只能是 `local` 或 `internal`
- `GAME_SERVER_INTERNAL_SOCKET_NAME` / `GAME_INTERNAL_SOCKET_NAME` 仅作为 local / development 非严格本地 fallback；strict registry disabled、registry client 不可用或没有合格 `game-server.internal` endpoint 时，建房返回 `RoomCreateFailed`，不会静默使用默认 socket
...
- 监控指标已接入，包含 QPS、延迟、池子大小和 `metrics:heartbeat:match-service`；该 heartbeat 只表示 metrics 新鲜度，不是 registry heartbeat，也不能替代 registry 服务发现
...
- M10 dashboard、告警与巡检不在本文当前完成态内，后续应在对应监控/运维任务中补充。
...
4. **MatchService 是否需要持久化匹配记录？**
   - 运行时恢复先使用 Redis；历史匹配记录建议后续沉淀到 PostgreSQL 审计表
