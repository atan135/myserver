...
- registry heartbeat 来自 service registry 的 `heartbeat:<service>:<instance>` 续约；metrics v2 latest `_reported_at` 只表示业务指标新鲜度，不能替代 registry 服务发现、endpoint 发布或 registry heartbeat。
...
- 监控指标已接入，包含 QPS、延迟和池子大小；指标新鲜度来自 v2 latest `_reported_at`，不是 registry heartbeat，也不能替代 registry 服务发现
