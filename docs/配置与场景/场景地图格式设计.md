...
`grassland_01` 当前同时承担 `world.main` / `main-world-public` 的主世界权威移动场景。当前服务端契约为：

- `scene_id=1`，范围为 `0 <= x < 4000`、`0 <= y < 4000`，上界 `4000` 为排他边界。
- `Width=40`、`Height=40`、`CellSize=100.0`，两层网格各包含 `1600` 项；当前 `walkable` 全为 `1`、`block` 全为 `0`。
- 100 米单元当前只表达全平地世界边界，不承诺单元内部障碍精度。
- 默认出生点 `1001` 为服务端坐标 `(2002,2002)`；当前客户端主世界原点映射后为 `(2,0,2)`。
- `SceneQuery` 对移动目标执行可行走校验和边界 clamp，客户端不能用本地坐标覆盖服务端权威位置。
...
对 `grassland_01`，`cell_size=100.0`，因此 `(3999.999,3999.999)` 仍位于最后一个单元，任一坐标为负数、非有限数或达到 `4000` 都不属于场景。移动系统通过 `SceneQuery::clamp_position` 保证权威实体不会推进到排他上界之外。
