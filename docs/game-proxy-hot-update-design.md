...
- 真实 old/new/proxy 多进程联调、mybevy 适配、旧服自动停止仍未完成。`complete-if-drained` 已具备可选旧服真实状态联动，但还不是完整自动停服控制面。
...
- 自动收尾可选读取旧服真实 `connection_count` / drain status；启用 `PROXY_ROLLOUT_DRAIN_STATUS_CHECK_ENABLED=true` 后，proxy route store 排空只是第一道条件，旧服真实 `connectionCount`、`ownedRoomCount`、`migratingRoomCount` 也必须为 `0` 才会结束 rollout。该能力仍不能单独代表旧服自动停进程控制面，停服前还需要控制面 owner、权限、审计和部署编排确认。
...
显式编排入口当前仍是第一阶段 redirect/reconnect 的前置控制流，不是同连接迁移。它只调用已鉴权 `game-server` admin TCP 包协议和 `game-proxy` admin HTTP；proxy 仍保持透明转发模型，不实现 L7 relay 或同连接 upstream swap。`tools/mock-client` 已有主动断线重连验证场景，旧服也会在 redirect push 成功排队后主动关闭旧连接，但这不是 old/new/proxy 多进程自动化联调，也不代表 mybevy 已适配。
...
4. 自动 rollout 结束检测已落地到 proxy route store 维度；旧服真实 drain/connection 状态已可通过 `auth-http` 内部接口和 mock-client 查询，下一步补自动联动、统一控制面 owner 和旧服自动停止。
