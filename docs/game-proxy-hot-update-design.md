...
- 真实 old/new/proxy 多进程联调、mybevy 适配、自动收尾与旧服真实状态的自动联动、旧服自动停止仍未完成。
...
- 自动收尾尚未自动读取旧服真实 `connection_count` / drain status；该真实状态目前可经 `auth-http` 内部控制接口和 `tools/mock-client --scenario rollout-drain-status` 人工查询，但仍不能单独作为旧服停进程自动化依据。
