...
- 真实 old/new/proxy 多进程联调、mybevy 适配、自动收尾与旧服真实状态联动、旧服自动停止仍未完成。
...
- 自动收尾尚未读取旧服真实 `connection_count` / drain status，因此不能单独作为旧服停进程依据。
...
4. 自动 rollout 结束检测已落地到 proxy route store 维度；下一步补旧服真实 drain/connection 状态联动、统一控制面 owner 和旧服自动停止。
