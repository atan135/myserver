...
4. 自动 rollout 结束检测已落地到 proxy route store 维度；旧服真实 drain/connection 状态已可通过 `auth-http` 内部接口和 mock-client 查询，game-server 受控 graceful shutdown 安全闸已可调用，下一步补统一控制面 owner、部署编排接入和真实旧进程自动停止。
