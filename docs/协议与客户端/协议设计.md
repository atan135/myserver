...
- `player_id` 当前仍是账号玩家 ID，对应 ticket payload 的 `playerId`。
- 角色身份来自 ticket payload 的 `characterId`，P0 阶段未新增 `AuthRes.character_id` 字段；服务端鉴权上下文和日志 / 审计上下文已同时携带账号 ID、角色 ID 和 `worldId`。
...
说明：P0 阶段该字段仍是账号玩家 ID，用于现有房间成员索引和断线重连查找，不是 `characterId`。
...
- `player_ids` 在 P0 阶段仍是账号玩家 ID 列表，尚未切换为角色 ID 列表
...
说明：`target_player_ids` 在 P0 阶段仍是账号玩家 ID。
...
说明：P0 身份拆分已经让鉴权上下文包含 `playerId`、`characterId`、`worldId`，但 `RoomMember.player_id` 和 room 成员索引仍是账号玩家 ID；后续若迁移为角色 ID，需要同步调整房间成员、输入、重连、迁移 payload 和离线索引。
...
- `playerId`：账号玩家 ID，仍用于账号级 session、封禁、限流、logout、改密、ticket version 和 P0 房间成员索引。
