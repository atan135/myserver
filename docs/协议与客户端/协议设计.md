...
同一条游戏连接上，请求响应和服务端 push 可能交错到达。客户端应按 `message_type + seq` 匹配请求响应；`RoomFrameRatePush`、`RoomStatePush`、`FrameBundlePush`、`ServerRedirectPush` 等 push 没有请求序列语义，必须作为异步事件处理。角色四属性查询结果、后续四属性变化 push 或其他状态刷新也应进入异步状态更新流程。不要假设发送某个请求后读取到的下一包一定是对应响应包。
