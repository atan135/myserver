...
| rulesFile | 必填 | 调用方 | agent 在 MYFORGE_ROOT 下读取的规则副本相对路径。P0 必须位于 rules/fangyuan/ 下并以 .md 结尾。 |
...
artifactFile、rulesFile 和可选 consumerTargetFile 的提交格式统一使用 /。三者都必须通过以下词法校验：
...
admin-api 做第一次词法校验，agent 必须独立做第二次相同校验。对会访问 MYFORGE_ROOT 的 artifactFile 和 rulesFile，agent 还必须：

1. 对 MYFORGE_ROOT 做 canonicalize，得到 rootReal。
2. rulesFile 必须已存在；canonicalize 后必须仍位于 rootReal 内。
3. artifactFile 的父目录必须已存在；canonicalize 父目录后必须仍位于 rootReal 内。
4. artifactFile 已存在时，也必须 canonicalize 文件本身并确认仍位于 rootReal 内。
5. 任一路径段经 symlink、junction 或 reparse point 解析到 rootReal 外时拒绝。
...
agent 不执行 commandPreview，也不使用 shell 拼接字符串。codex_exec profile 等价于 Rust 的直接进程调用：
...
P0 不从消息读取 executable、argv、cwd、环境变量、sandbox、model、profile 配置名或 additional writable directory。不得加入 --dangerously-bypass-approvals-and-sandbox、--add-dir，或把 MYFORGE_ROOT 之外的目录设为可写。
...
commandPreview 可以按相同固定参数生成人类可读文本，但该文本不要求可被 shell 反向解析，且永远不能成为执行输入。
...
dry-run 收到 command.execute 后仍必须完成验签、TTL、防重放、requestId 幂等、MYFORGE_ROOT、rulesFile、artifactFile 父目录和 typed input 全部校验。校验通过后：
...
MYFORGE_ENABLED、MYFORGE_DRY_RUN、MYFORGE_AUDIT_ENABLED 不得复用会把未知字符串静默转成 false 的宽松 parseBoolean。三者使用独立 strictBoolean：

- 只有环境变量 key 完全缺失时使用默认值；三者默认均为 false。key 已存在但去除首尾 ASCII whitespace 后为空属于非法配置。
...
- 尤其 MYFORGE_DRY_RUN 的非法值必须 fail closed，不能回退为 false 后执行真实 Codex。

配置测试至少覆盖缺失默认、空字符串拒绝、四个合法值、大小写错误和 MYFORGE_DRY_RUN=tru。错误日志只记录变量名和“invalid boolean”，不回显其他配置。
...
| MYFORGE_COMMAND_TIMEOUT_MS | server | 120000 | 1000..1800000 |
| MYFORGE_MAX_COMMAND_TIMEOUT_MS | agent | 120000 | 1000..1800000 |
...
platform 只允许 windows、linux、macos。audit 只允许 available 或 unavailable。dryRun 必须等于当前进程 MYFORGE_DRY_RUN 的解析结果，连接存续期间不得切换。forgeRootSummary 不得包含绝对路径；name 是 MYFORGE_ROOT 的末级目录名，长度 1 至 128 字节。register 的 expiresAtMs-timestampMs 与 hello 使用同一个 agentHelloRegisterTtlMs。limits 是 agent 本地已验证上限的只读快照，参与签名；server 在把连接标记 registered 前完成协商并把 effective limits 保存在连接上下文。server 只保存和展示摘要。

P0 maxConcurrentTasks 固定为 1。capabilities.codexExec 是 binary probe 的真实结果；dryRun=false 时必须为 true，dryRun=true 时可为 true 或 false。capabilities 只用于调度和展示，不能扩大 server 允许的 profile。
...
consumerTargetFile 未提交时必须在签名消息中显式为 null。timeoutMs 和 maxOutputBytes 必须与当前 connection effectiveLimits.commandTimeoutMs/maxOutputBytes 完全相同；仅“小于本地上限”不足以通过，防止两端对 task watchdog 和截断边界理解不同。不匹配返回 MYFORGE_LIMIT_MISMATCH。cwd、command、args 和 shell 字段在该 schema 中不存在。
...
| skipped | null | null | null | dry_run / execution_failed / artifact_missing / cancelled | 空数组 |
...
| Codex 退出 0 但 artifact 缺失 | failed | 0 | 必填整数 | exists=false | skipped，reasonCode=artifact_missing | MYFORGE_TARGET_FILE_MISSING | 非空 |
...
子进程 spawn/preflight 失败使用 command.error，不发送 result。completed_with_errors 只用于审核 warning/failed，不用于执行失败。正常输出截断只设置 truncated，不改变 status。除 completed 外所有 result 的 errorCode 和 errorMessage 都必须非 null；completed 两者必须为 null。
...
审计只记录 requestId、taskType、agentId、projectId、状态、路径摘要、错误码、管理员和必要 timing，不记录私钥、公钥正文、完整 signature、完整 renderedPrompt 或完整 stdout/stderr。
...
返回 items、total、limit、offset。列表项包含 requestId、taskType、projectId、agentId、status、queueReason、executionMode、artifactFile、consumerTargetFile、createdBy、createdAt、dispatchedAt、startedAt、cancelRequestedAt、cancelDeadlineAt、completedAt、durationMs、errorCode，不返回 stdout/stderr 和 renderedPrompt 全文。
...
真实 codex_exec 退出 0 且 artifact 存在时，audit available 的 agent 使用直接进程参数调用：
...
- skipped：本次有明确不运行原因，只允许 dry_run、execution_failed、artifact_missing、cancelled。它不表示本地缺少审核器。
...
- prompt 是受限业务数据，但 Codex 仍是可写 MYFORGE_ROOT 的自动化执行器；myforge 工作区不得存放生产密钥或无关敏感文件。
...
- agent 通过直接进程参数调用 Codex，cwd 固定为 canonical MYFORGE_ROOT，commandPreview 永不执行。
- artifactFile 和 rulesFile 是 MYFORGE_ROOT 内受控相对路径；consumerTargetFile 只作元数据。
