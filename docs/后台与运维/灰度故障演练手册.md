...
- 该 CLI 不提供真实写执行；真实 room transfer/redirect 必须调用 admin-api 控制面两步协议。
...
| `route-metadata-missing` | `proxy_route_upsert` | simulate 模式在内存 mock 中验证既有 room route metadata 缺失时的安全闸。 | 停在 `proxy_route_upsert`；不调用 `/room-route/upsert` 创建新 route，不 retire old room。 |
...
故障演练 CLI 只负责 dry-run/simulate，不再直接连接 game-server 或 game-proxy 管理口。真实 room transfer 必须先通过 admin-api 控制面完成两步预检/执行，再用本 CLI 做结果归档或在受控测试桩上模拟故障阶段。
...
node tools/rollout/rollout-fault-drill-cli.js ^
  ...
  --archive-file artifacts/rollout/import-failure.json
```

`route-metadata-missing` 的 simulate 模式只在内存测试桩中验证缺失 metadata 的安全闸。真实环境的 route 查询、恢复和迁移必须由 admin-api 控制面及其 registry 目标完成。
...
```bash
node tools/rollout/rollout-control-plane-cli.js ^
  --admin-api-url http://127.0.0.1:3001 ^
  --admin-api-token "<admin-api-jwt>" ^
  --world-id world-1 --rollout-epoch rollout-20260612-route-missing ^
  --room-id room-empty-001 --old-server-id game-server-old ^
  --new-server-id game-server-new --proxy-instance-id game-proxy-1 ^
  --backup-reference rollout-route-metadata-recovery ^
  --reason "route metadata recovery drill" --execute
```
...
- admin-api 操作审计中没有成功的 `proxy.route.write`；old game-server 没有成功 retire。
...
1. 人工恢复旧 metadata 后重新执行。
   ...
   - 路由恢复必须通过受控的 admin-api/proxy route control-plane 接口并使用 CAS；不得携带通用 proxy token 直接写管理口。
   - 恢复后重新运行 `rollout-control-plane-cli.js` 的预检/执行，确认 route metadata 已恢复后再进入正常迁移。
...
本工具只处理 Room rollout，不处理邮件链路，不能用本文的 room simulate 结果替代邮件验收。邮件通知/附件领取的 NATS 中断、响应丢失、mail-service 崩溃、权威实例切换和数据库/Redis 故障注入已由 `tests/mail/mail-reliability-fault-drill.test.mjs` 独立完成，11 项测试全部通过。邮件部署、停止新领取、租约接管和 fail-closed 回滚步骤见 [邮件可靠链路灰度与回滚手册](./邮件可靠链路灰度与回滚手册.md)。
