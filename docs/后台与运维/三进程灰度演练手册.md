...
本文说明 `scripts/ops/rollout-three-process-drill.ps1` 的使用方式。该脚本是第一阶段可重复演练入口，用来把已经落地的控制面工具串起来：
...
- Redis / PostgreSQL / NATS 等依赖按当前环境要求启动。
- `auth-http` 已运行，且内部 game-server admin client 可通过 registry 发现 old game-server。
...
- `game-proxy` 已运行，admin endpoint 已注册为 `game-proxy.admin`，并能通过 `game-server.proxy-local` 发现 old/new 两个 upstream。
- test / staging / production 必须启用 `REGISTRY_ENABLED=true`、`DISCOVERY_REQUIRED=true`，并通过 service registry endpoint name 和 instance id 解析 `auth-http.internal`、`game-proxy.admin`、old/new `game-server.admin`。固定 host/port 只允许 local/manual fallback。
...
不要把历史 Unity demo 当正式客户端准备工具；该 demo 已从本仓库删除。正式客户端在外部 `mybevy` 仓库，本仓库侧演练使用 `tools/mock-client`。
...
```powershell
node tools/rollout/rollout-transfer-cli.js --dry-run `
...
该 CLI dry-run 只做参数和计划校验，不打开 game-server admin socket，不访问 proxy HTTP，也不会 retire / shutdown。上面的端口和 URL 是本地 fallback 示例；测试、预发和线上应先让外层脚本从 registry 发现 endpoint，再把发现结果传给底层 CLI。输出 JSON 中的关键字段：
...
| `-OldAdminHost` | `MYSERVER_OLD_GAME_ADMIN_HOST` | `127.0.0.1`，仅 local/manual fallback |
| `-OldAdminPort` | `MYSERVER_OLD_GAME_ADMIN_PORT` | `7500`，仅 local/manual fallback |
| `-AdminApiToken` | `ADMIN_API_TOKEN` | admin-api JWT，仅控制面预检/执行 |
| `-NewAdminHost` | `MYSERVER_NEW_GAME_ADMIN_HOST` | `127.0.0.1`，仅 local/manual fallback |
| `-NewAdminPort` | `MYSERVER_NEW_GAME_ADMIN_PORT` | `7501`，仅 local/manual fallback |
| `-WorldId` | `MYSERVER_WORLD_ID` | 控制面目标 world |
| `-ProxyAdminUrl` | `MYSERVER_PROXY_ADMIN_URL` | `http://127.0.0.1:7101`，仅 local/manual fallback |
| `-ProxyInstanceId` | `MYSERVER_PROXY_INSTANCE_ID` | registry 中的 game-proxy 实例 |
| `-AuthBaseUrl` | `MYSERVER_AUTH_BASE_URL` | `http://127.0.0.1:3000`，仅 local/manual fallback |
...
底层 `rollout-control-plane-cli.js` 使用 admin-api JWT 执行两步预检/执行，不再接受 old/new/proxy admin token 或 actor 参数；下游 game-server/game-proxy 写请求由 admin-api 签发短时 assertion。
...
第 4 项故障恢复演练不走 `scripts/ops/rollout-three-process-drill.ps1` 的默认成功迁移路径。主 agent 需要先在受控测试环境中制造目标 room 的 proxy route metadata 缺失，例如使用 Redis route store 快照备份后移除对应 `room_routes.<room_id>`，或在内存 route store 测试环境中通过重启/重置 route store 制造缺失。制造前必须归档 `GET /room-routes`、old drain status 和 Redis route store 原始快照，方便恢复。

缺失制造完成后先用 fault drill simulate 验证安全闸；真实迁移使用 control-plane CLI:
...
恢复策略见 [rollout 故障演练入口](./灰度故障演练手册.md) 的 “route metadata 缺失后的恢复策略”。在缺失 metadata 未恢复并复核前，不要继续 `old_retire`，不要执行 `complete-if-drained`。

最近一次人工验收记录已经沉淀到本文和 [生产拓扑与 Room 迁移设计](./生产拓扑与Room迁移设计.md)：该次验收使用 Redis、Core NATS、auth-http、old/new game-server 和 game-proxy，跑通 old freeze/export、new import/confirm ownership、proxy route upsert、old retire 和 `complete-if-drained`；未传 `-AllowShutdownRequest`，因此没有请求自动停旧服。
