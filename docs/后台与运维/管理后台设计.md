...
P2/P3/P4 后台仍不实现授予、撤销、装备称号、激活 / 停用 / 切换职业、推进职业 points 或触发正式进度奖励等写操作，这些写操作保留给正式玩家入口或后续 GM 闭环阶段。本阶段 `admin-web` 不新增角色称号页面，只完成 `admin-api` 查询能力；如需运营页面，应在后续阶段补充加载、空状态、错误状态、过期称号展示、职业 active 状态展示和正式进度日志筛选。
