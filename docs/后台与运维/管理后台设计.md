...
P2 不实现后台授予、撤销、装备称号或修改职业阶位等写操作，这些写操作保留给后续 GM 闭环阶段。本阶段 `admin-web` 不新增角色称号页面，只完成 `admin-api` 查询能力；如需运营页面，应在后续阶段补充加载、空状态、错误状态和过期称号展示。
