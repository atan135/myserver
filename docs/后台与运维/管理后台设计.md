...
- `game-server` admin TCP 通道：测试/线上通过 service registry 发现目标实例 endpoint 后执行 GM 发道具等指令；GM 广播仅在全局 NATS 发布失败时作为 legacy 单实例 fallback，踢人/封禁的单实例调用保留为 legacy 兼容和辅助审计。本地开发可保留固定 host/port 作为手工 fallback，不作为测试/线上消费者配置
...
# 本地开发手工 fallback；测试/线上应通过 service registry 发现 game-server admin endpoint
# GAME_SERVER_ADMIN_HOST=127.0.0.1
# GAME_SERVER_ADMIN_PORT=7500
GAME_ADMIN_TOKEN=dev-only-change-this-game-admin-token

# 本地开发手工 fallback；测试/线上应通过 service registry 或部署控制面发现 game-proxy admin endpoint
...
- 测试/线上 `admin-api` 应通过 service registry 发现 `game-server` admin endpoint 后做 GM 指令转发；`GAME_SERVER_ADMIN_HOST` / `GAME_SERVER_ADMIN_PORT` 仅作为本地开发或 registry 关闭时的手工 fallback，不能作为测试/线上消费者配置
- 本地开发 fallback 可对接 `game-server` admin 默认入口端口 `7500`
- 测试/线上 `admin-api` 应通过 service registry 或部署控制面发现 `game-proxy` admin endpoint 后查询 rollout / drain 状态；`GAME_PROXY_ADMIN_HOST` / `GAME_PROXY_ADMIN_PORT` 仅作为本地开发或 registry 关闭时的手工 fallback
