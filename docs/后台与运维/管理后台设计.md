...
称号定义展示字段来自 `apps/game-server/csv/TitleTable.csv`，包括名称、类型、稀有度、图标、颜色、隐藏标记、限时标记和排序值。`equippedTitle` 只返回当前未过期且装备中的称号；过期称号仍保留在 `titles` 中并带 `expired=true`，便于运营识别异常装备状态和同名角色。

P2/P3 后台仍不实现授予、撤销、装备称号、激活 / 停用 / 切换职业或推进职业 points 等写操作，这些写操作保留给后续 GM 闭环阶段。本阶段 `admin-web` 不新增角色称号页面，只完成 `admin-api` 查询能力；如需运营页面，应在后续阶段补充加载、空状态、错误状态、过期称号展示和职业 active 状态展示。
