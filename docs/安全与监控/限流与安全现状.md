...
下面 host/port 是服务自身本地监听默认值或本地 fallback 示例；测试/线上消费者发现上游和 admin endpoint 时应走 service registry 或部署控制面，不应硬编码这些值。
...
# registry 关闭时的本地开发兼容配置；测试/线上不应依赖静态上游
...
- proxy admin 持久审计是控制面补充，不改变网络边界：生产环境仍必须通过私网、VPN、堡垒机、安全组或等价机制隔离 proxy admin endpoint，不能据此把 admin 口暴露公网；`PROXY_ADMIN_HOST` / `PROXY_ADMIN_PORT` 仅作为本地开发或手工 fallback 的静态配置。
...
`CHAT_BIND_ADDR` 是 `chat-server` 自身监听地址示例；客户端或其它服务在测试/线上应使用登录响应或 service registry 返回的 endpoint。
