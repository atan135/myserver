...
Core NATS、`chat:online`、proxy route store、`metrics:*` / `metrics:heartbeat:*` 都不是 service registry。它们不能替代 endpoint discovery、鉴权授权或 registry heartbeat；当前 metrics 采集、分钟归档和后台监控页面也不表示 M10 的统一告警、审计化自动巡检或容量门禁已经完成。
...
- service registry 与监控边界：
  ...
  - 当前已具备 metrics 采集、分钟归档和后台监控页面，不声明 M10 的统一告警、审计化自动巡检或容量门禁已经完成
