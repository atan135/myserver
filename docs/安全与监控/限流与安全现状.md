...
测试、预发和线上跨服务消费者必须通过 Redis service registry 的 endpoint name 与实例信息发现目标服务，不应把固定 host/port/socket 作为准入或推荐路径。`REGISTRY_ENABLED=false`、固定端口表和 `.env.example` 中的地址只表示本地默认监听或本地调试示例；在 `DISCOVERY_REQUIRED=true`，或 `NODE_ENV` / `APP_ENV` 为 `test`、`staging`、`prod`、`production` 等严格环境时，旧直连 fallback 应禁用、忽略并告警，或在配置加载阶段 fail-fast。

只允许在 `development` / `local` 且 discovery 非 required 的本地单机调试中使用旧直连变量，例如 `GAME_SERVER_ADMIN_HOST` / `GAME_SERVER_ADMIN_PORT`、`GAME_PROXY_ADMIN_HOST` / `GAME_PROXY_ADMIN_PORT`、`MATCH_SERVICE_ADDR`、`UPSTREAM_LOCAL_SOCKET_NAME`、`GAME_SERVER_INTERNAL_SOCKET_NAME` / `GAME_INTERNAL_SOCKET_NAME`。这些变量不能作为测试、预发或线上链路通过条件。

service registry 只解决“发现哪个 endpoint”，不替代安全控制。`game-server.admin`、`game-proxy.admin` 以及 `game-server.internal`、`game-server.proxy-local` 等 admin / internal / local endpoint 即使被发现，也仍需内网或同机隔离、token / RBAC、审计、IP allowlist、TLS / mTLS 或等价部署保护。

正式客户端默认只接触 `auth-http` 和 `game-proxy` 玩家入口。`chat-server`、`mail-service`、`announce-service`、`match-service` 默认是内网能力服务；`auth-http` 可选返回 `chat` / `mail` / `announce` endpoint 仅限显式允许的非生产场景，并且 endpoint 来源应是 registry，不应编造本地 fallback。

Core NATS、`chat:online`、proxy route store、`metrics:*` / `metrics:heartbeat:*` 都不是 service registry。它们不能替代 endpoint discovery、鉴权授权或 registry heartbeat；当前 metrics 采集和后台读取链路也不表示 M10 dashboard、alerts 或审计化巡检已经完成。
...
- 安全审计：登录失败、账号锁定、IP 限流、ticket 撤销等事件会写入 `security_audit_logs`（前提是启用了 PostgreSQL 存储）
...
- `TICKET_VALIDATE_ENABLED` 和 `SECURITY_AUDIT_ENABLED` 已进入配置结构，但当前代码没有完整用它们做开关控制：
  ...
  - 安全审计当前由数据库 store 直接写库，未额外判断 `SECURITY_AUDIT_ENABLED`
...
下面 host/port/socket 是服务自身本地监听默认值或 Local fallback only 示例；测试、预发和线上消费者发现上游和 admin endpoint 时必须走 service registry 或部署控制面，不应硬编码这些值。

```env
PROXY_HOST=127.0.0.1
PROXY_PORT=4000
# 本地开发/手工 fallback；测试/线上消费者通过 service registry 或部署控制面发现 admin endpoint
...
# Local fallback only：仅 development/local 且 discovery 非 required 时可用；
# strict/test/staging/prod/production 或 DISCOVERY_REQUIRED=true 下禁用或 fail-fast
...
- 当前 proxy admin 修改接口会记录 action、操作人、关键目标和 ok/error 结果到结构化日志，并在 `PROXY_ADMIN_AUDIT_ENABLED=true` 时追加 JSONL 持久审计。操作人来自 `X-Admin-Actor`，缺失时记录为 `unknown` 且 `actor_missing=true`；`PROXY_ADMIN_AUDIT_REQUIRE_ACTOR=true` 时缺失 actor 的写操作会被拒绝。JSONL 写入失败按安全优先拒绝写操作并记录 warning。当前仍未接入 PostgreSQL 审计查询，也没有更细操作级 RBAC。
- `PROXY_ADMIN_AUDIT_ENABLED=true` 默认开启轻量文件审计，`PROXY_ADMIN_AUDIT_PATH` 默认 `logs/game-proxy/admin-audit.jsonl`。本地可关闭；生产应覆盖为 `/var/log/myserver/admin-audit.jsonl`，挂载独立的 `game-proxy-audit` volume，并按[服务端日志采集与留存设计](./服务端日志采集与留存设计.md)保持 audit 与普通 Vector `docker_logs` 链路分离。
- proxy admin 持久审计是控制面补充，不改变网络边界：生产环境仍必须通过私网、VPN、堡垒机、安全组、IP allowlist、TLS / mTLS 或等价机制隔离 proxy admin endpoint，不能据此把 admin 口暴露公网；`PROXY_ADMIN_HOST` / `PROXY_ADMIN_PORT` 仅作为 `development` / `local` 且 discovery 非 required 时的手工 fallback 静态配置。
...
- `game-server` 的 `GAME_ADMIN_TOKEN` 与 `GAME_INTERNAL_TOKEN` 用于内部管理/服务通道；生产拒绝默认值或空值，并要求 admin TCP JSONL 审计保持启用，但仍应依赖私网、IP allowlist、TLS/mTLS 或同机 socket 等部署侧隔离。测试、预发和线上调用 `game-server.admin` / `game-server.internal` 应通过 service registry 发现 endpoint，不能默认直连 `127.0.0.1:7500` 或固定 socket
...
`CHAT_BIND_ADDR` 是 `chat-server` 自身监听地址示例；`chat-server` 默认是内网能力服务。客户端或其它服务在测试、预发和线上应使用登录响应或 service registry 返回的 endpoint，不应把 `0.0.0.0:9001` / `127.0.0.1:9001` 作为直连建议。
...
REDIS_KEY_PREFIX=
DB_ENABLED=true
DATABASE_URL=postgres://postgres:password@localhost:5432/myserver_chat
DB_POOL_SIZE=5
...
- `chat-server` 默认不作为测试、预发或生产公网入口；上述 fail-fast 只是内网服务凭证保护。只有本地开发或手工调试 fallback 可以临时直连，且仍应继续保持与 `game-proxy` / `game-server` 一致的 ticket 校验边界
...
- 拒绝事件会写入结构化安全日志；当前不写入 PostgreSQL `security_audit_logs`
...
`ADMIN_API_REQUIRE_TLS` 开发默认 `false`，`NODE_ENV=production` 默认 `true`；经反向代理部署时，必须同时正确配置 `TRUST_PROXY` / `TRUSTED_PROXIES`，否则不可信来源伪造的 `X-Forwarded-Proto=https` 不会生效。`ADMIN_API_REQUIRE_IP_ALLOWLIST=false` 默认关闭，避免破坏本地联调；生产建议结合部署侧网络隔离启用。`GAME_ADMIN_CONNECT_TIMEOUT_MS`、`GAME_ADMIN_WRITE_TIMEOUT_MS`、`GAME_ADMIN_READ_TIMEOUT_MS` 和 `GAME_ADMIN_MAX_RESPONSE_BYTES` 缺失、非法、0 或负数时回退到默认值。代码侧保护不代表 `admin-api/admin-web` 可以作为生产公网玩家入口；`admin-api` 调用 `game-server.admin` / `game-proxy.admin` 时，测试、预发和线上应经 service registry 发现 endpoint，本地固定 host/port 只允许 development/local fallback。
...
- `auth-http`：
  ...
  - 使用 `AUTH_REQUIRE_TLS` 控制请求级 HTTPS/TLS 强制；开发/测试默认 `false`，`NODE_ENV=production` 默认 `true`
  - 使用 `AUTH_REGISTER_REQUIRE_REVIEW` 控制密码账号注册后是否进入 `pending_review` 审核状态；默认 `false`
  - 使用 `TRUST_PROXY`、`TRUSTED_PROXIES` 控制真实来源 IP 与 `X-Forwarded-Proto` 解析；只有可信代理来源才会信任 forwarded header，`TRUSTED_PROXIES` 支持精确 IP 和 IPv4 CIDR
  - 登录响应中的游戏入口应来自 `game-proxy.client` registry endpoint；`GAME_PROXY_HOST` / `GAME_PROXY_PORT` 只允许 development/local 且 discovery 非 required 的兼容 fallback
  - 可选暴露 `chat-server`、`mail-service`、`announce-service` endpoint 仅限显式允许的非生产场景，并且来源应是 registry；生产默认不暴露这些内部 endpoint
...
- `game-proxy`：
  ...
  - 上游 `game-server` 应通过 `game-server.proxy-local` registry endpoint 发现；`UPSTREAM_LOCAL_SOCKET_NAME` 只允许 development/local 且 discovery 非 required 的静态 upstream fallback
- `game-server`：
  ...
  - 匹配客户端应通过 `match-service.grpc` registry endpoint 发现；`MATCH_SERVICE_ADDR` 只允许 development/local 且 discovery 非 required 的本地 fallback
  - `game-server.admin` 和 `game-server.internal` 即使通过 registry 发现，也仍是内网 / local 控制面 endpoint，必须继续依赖 token、审计、网络隔离、allowlist 和 TLS / mTLS 等保护
- `chat-server`：
  ...
  - 默认是内网能力服务；测试、预发和线上不应把 `CHAT_BIND_ADDR` 固定地址作为正式客户端直连入口
- `admin-api`：
  ...
  - 调用 `game-server.admin` 和 `game-proxy.admin` 应通过 service registry 发现；`GAME_SERVER_ADMIN_HOST` / `GAME_SERVER_ADMIN_PORT`、`GAME_PROXY_ADMIN_HOST` / `GAME_PROXY_ADMIN_PORT` 只允许 development/local 且 discovery 非 required 的本地 fallback
- `mail-service`：
  - 默认是内网能力服务；玩家读邮件和附件领取虽已有 game ticket 校验，但测试、预发和线上不应把固定 HTTP host/port 作为正式客户端直连建议
  - 发放附件调用 `game-server.admin` 应通过 service registry 发现；`GAME_SERVER_ADMIN_HOST` / `GAME_SERVER_ADMIN_PORT` 只允许 development/local 且 discovery 非 required 的本地 fallback
- `match-service`：
  - 默认是内网 gRPC 能力服务；`MATCH_BIND_ADDR` / `9002` 只表示本地默认监听，不是测试、预发或线上消费者直连建议
  - 创建房间调用 `game-server.internal` 应通过 registry 发现 local/internal endpoint；`GAME_SERVER_INTERNAL_SOCKET_NAME` / `GAME_INTERNAL_SOCKET_NAME` 只允许 development/local 且 discovery 非 required 的本地 fallback
- `announce-service`：
...
- service registry 与监控边界：
  - `metrics:heartbeat:*`、Core NATS、`chat:online` 和 proxy route store 不是 service registry，不能替代 endpoint discovery、鉴权授权或 registry heartbeat
  - 当前只说明 metrics 采集和基础后台读取链路，不声明 M10 dashboard、alerts、审计化巡检或容量指标已经完成
