...
本文不是“当前已经全部实现”的说明，而是开发决策参考。当前实现状态仍以代码、[安全设计](./安全设计.md) 和 [限流与安全现状](./限流与安全现状.md) 为准。
...
后台直接调整角色四属性、称号和职业阶位属于高风险敏感操作，最低要求：

- 后端权限必须使用明确权限点，例如 `gm.character_elements.write`、`gm.character_titles.write`、`gm.character_disciplines.write`。
- 请求必须包含 `reason`，并记录操作者、来源 IP、目标 `character_id`、before/after、稳定错误码。
- 四属性写操作必须同步写 `character_element_logs`；称号写操作必须同步写 `character_title_logs`；职业阶位写操作必须同步写 `character_discipline_logs`。
- 后台审计必须写 `admin_audit_logs`；权限不足由 `security_audit_logs` 记录。
- 重复提交同一目标状态应幂等，不重复制造业务效果，但可以保留 0 delta 或 unchanged 日志作为操作留痕。
- 限时称号授予必须显式提供过期时间；自动解锁检查不得默认授予活动、排行、系统手工类称号。
