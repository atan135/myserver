...
- [协议设计](../协议与客户端/协议设计.md)
- [帧同步与房间生命周期设计](../游戏服与接入层/帧同步与房间生命周期设计.md)
- [场景地图格式设计](../配置与场景/场景地图格式设计.md)
- [底层框架路线图](../游戏服与接入层/游戏服底层框架路线图.md)
- [生产拓扑与 Room 迁移设计](../后台与运维/生产拓扑与Room迁移设计.md)
- [服务注册中心设计](../周边服务/服务注册中心设计.md)
- [服务发现迁移基线](../周边服务/服务发现迁移基线.md)

说明：

- 本文中的“当前已实现”以仓库现状为准
- 本文中的“建议新增配置项”只是设计建议，当前不表示代码已经读取
- `docs/历史归档/初始设计稿/*` 中的提示词不视为正式设计文档，正式口径以本文和现状文档为准
...
5. 数据与凭证边界：Redis、NATS、PostgreSQL、`.env` 密钥、日志与审计库

生产公网暴露总口径以 [生产拓扑与 Room 迁移设计](../后台与运维/生产拓扑与Room迁移设计.md) 为准：正式玩家客户端只应依赖 `auth-http` 和 `game-proxy`，其它业务服务默认内网化。内部服务固定 host/port 只允许作为本地开发或手工调试 fallback；测试、预发和线上必须通过部署入口、registry endpoint 或 instance id 解析目标，不能把直连内部服务作为准入路径。服务注册中心只改变“如何发现目标实例”，不改变“谁可以访问该 endpoint”的公网暴露边界。

### 2.2 服务发现与安全边界

当前统一 service registry 只负责服务实例和 endpoint 发现，不负责授权、鉴权、ACL、网关、TLS 终止，也不授予公网暴露许可。某个 endpoint 被注册到 Redis registry，只表示合规消费者可以按 endpoint name 和 instance id 找到它；是否可访问仍由部署网络、网关、证书、token、RBAC、IP allowlist 和审计策略共同决定。

`public` / `internal` / `admin` / `local` visibility 是发现语义和运维语义边界，不是单独的安全控制：

- `public` endpoint 仍必须通过 HTTPS/TLS、安全组、防火墙、网关、限流和 ticket/token 校验保护。
- `internal` endpoint 默认只允许内网能力服务或指定工作负载访问，不能因为可被 registry 发现就暴露给公网。
- `admin` endpoint 只属于控制面 / 运维面，例如 `game-server.admin`、`game-proxy.admin` 和 `admin-api`；它们必须通过运营网段、堡垒机、VPN、security group、IP allowlist、TLS、token/RBAC 和审计保护，不能因为注册到 registry 就对公网开放。
- `local` endpoint 是同机或受限工作负载通道，例如 `game-server.internal`、`game-server.proxy-local` / `proxy-local` local socket；socket 路径、service token、`GAME_INTERNAL_TOKEN` 等仍属于敏感凭证和高权限入口，必须按同机/内网范围隔离。

正式客户端默认只应接触 `auth-http` 和 `game-proxy` 玩家入口。`chat-server`、`mail-service`、`announce-service`、`match-service` 默认是内网能力服务；`auth-http` 可在显式允许的非生产配置下经 registry 返回 `chat` / `mail` / `announce` 等内部 endpoint，但生产默认不返回这些 endpoint，也不能把这些 endpoint 作为客户端公网准入路径。

控制面写操作仍必须保留认证、授权、审计和目标实例策略。`admin-api` / `mail-service` 等消费者通过 registry 发现 `game-server.admin` 或 `game-proxy.admin` 后，单实例写操作仍需要明确 `targetInstanceId`、owner 或等价目标策略，不能在多个 endpoint 中随机写入；注册中心不替代后台权限矩阵、service token、proxy scoped token、JSONL / PostgreSQL 审计或操作审批。

旧 host/port/socket fallback 只允许 `development` / `local` 且 discovery 非 required 的本地联调或手工 drill。`test`、`staging`、`prod`、`production` 或 `DISCOVERY_REQUIRED=true` 下，不允许使用 `GAME_PROXY_HOST`、`GAME_SERVER_ADMIN_HOST`、`GAME_PROXY_ADMIN_HOST`、`MATCH_SERVICE_ADDR`、`UPSTREAM_LOCAL_SOCKET_NAME`、`GAME_SERVER_INTERNAL_SOCKET_NAME` 等旧直连变量绕过 registry，也不能把这些固定地址当作安全暴露边界依据。

需要额外区分以下 Redis / NATS 数据面：

- Core NATS 的邮件通知、GM 广播、session kick 和 metrics 上报不是 service discovery，也不替代认证、授权或 registry heartbeat。
- `chat:online`、proxy route store、`metrics:*`、`metrics:heartbeat:*` 是业务路由、在线状态或指标新鲜度数据，不是 service registry schema，也不能用来推导 admin/internal endpoint 地址。
- Registry heartbeat 只表示实例发现数据的新鲜度；它不是健康准入、告警闭环或 M10 dashboard / alerts / 审计化巡检完成的证明。
...
| 模块 | 当前已实现 | 当前缺口 |
|------|------------|----------|
| `auth-http` | IP 限流、Redis 动态 IP / 玩家黑名单、账号锁定、ticket 签发与撤销、维护模式下拦截普通玩家登录和新 game ticket 签发、GM 限时封禁到期后在登录/签票路径惰性恢复账号为 `active`、内部接口可选 service token、安全审计写库、请求级 HTTPS/TLS 强制、可信代理来源 IP 与 `X-Forwarded-Proto` 解析；production 下拒绝默认 `TICKET_SECRET`、默认 `GAME_ADMIN_TOKEN` 和空 `INTERNAL_API_TOKEN` | ticket 仍为跨服务复用票据，尚未做用途隔离、换票或重放窗口收敛；代码侧 TLS 强制不能替代生产网关/证书/安全组配置 |
...
- [限流与安全现状](./限流与安全现状.md)：已实现的限流、ticket、安全边界
- [管理后台设计](../后台与运维/管理后台设计.md)：管理员鉴权、审计表、监控接口现状
...
6. 管理口、监控口、Redis、PostgreSQL 等控制面被误暴露到公网
...
| 链路 | 当前现状 | 目标策略 |
|------|----------|----------|
| 客户端 -> `auth-http` | 开发/测试期可明文 HTTP；`AUTH_REQUIRE_TLS=true` 时非 HTTPS 返回 `426 AUTH_TLS_REQUIRED` | 生产必须 HTTPS，可由网关/LB 终止 TLS 后转发内网 HTTP |
| 客户端 -> `mail-service` | 仅本地开发或手工调试 fallback 可直连明文 HTTP；玩家接口默认要求 game ticket，发信入口要求 `MAIL_SERVICE_TOKEN` | 测试、预发和线上不以客户端直连作为准入路径；若临时暴露，必须 HTTPS，并继续收敛到可信网关、用途隔离 ticket 或换票模型 |
| 客户端 -> `announce-service` | 仅本地开发或手工调试 fallback 可直连明文 HTTP；只读查询默认要求 `ANNOUNCE_READ_TOKEN` 或 game ticket；写接口要求 `ANNOUNCE_ADMIN_TOKEN` header | 测试、预发和线上不以客户端直连作为准入路径；若临时暴露，必须 HTTPS，并继续收敛到可信网关、用途隔离 ticket 或换票模型，后台 CRUD 继续与玩家读取路径隔离 |
| 浏览器 -> `admin-web` / `admin-api` | 当前未强制 HTTPS | 生产必须 HTTPS，Bearer token 只允许在 TLS 下使用，并限制在运营网段、堡垒机、VPN 或独立管理入口 |
| 客户端 -> `chat-server` TCP | 仅本地开发或手工调试 fallback 可直连明文 TCP | 测试、预发和线上不以客户端直连作为准入路径；若临时暴露，必须在入口层做 TLS 终止或由 `chat-server` 直接支持 TLS |
...
- 生产环境必须替换默认示例密钥
- `auth-http` 在 `NODE_ENV=production` 或 `APP_ENV=production` 时会在配置加载阶段 fail fast：默认或空的 `TICKET_SECRET`、默认或空的 `GAME_ADMIN_TOKEN`、空的 `INTERNAL_API_TOKEN` 都会拒绝启动。`AUTH_REQUIRE_TLS` 开发默认 `false`、production 默认 `true`；经反向代理部署时，只有 `TRUST_PROXY=true` 且直连来源命中 `TRUSTED_PROXIES` 后，`X-Forwarded-Proto=https` 才被视为安全请求。`AUTH_STRICT_SECURITY` 仍控制内部接口请求期缺 token 时的拒绝行为，但生产环境不再等到请求阶段才暴露空 token 配置错误。
...
普通服务运行日志继续由服务 console 输出，并由 Docker 提供短期缓冲；已纳入 release bundle 的独立常驻 Vector 按服务、实例和 UTC 日期写入 `/data/myserver/log`。这条链路用于运行时排障，不承担不可抵赖的管理或安全审计；Vector 未在目标宿主机启动或输出不可用时，仅以 Docker `local` driver 的 `docker logs` 窗口兜底，不能把 audit 或 metrics 当作普通日志事实源。

`admin_audit_logs`、`security_audit_logs`、`game_connection_audit_logs` 等审计事实源必须保持独立，不能用普通运行日志替代；同样，metrics-collector 的指标数据也不属于普通日志采集链路。详细目录、补采、脱敏和丢失边界见[服务端日志采集与留存设计](./服务端日志采集与留存设计.md)。
...
- Redis、PostgreSQL 不直接暴露到公网
- `game-server` admin 端口不对公网开放
- `game-proxy` admin 端口不对公网开放
- `game-server.admin`、`game-proxy.admin`、`admin-api` 等 admin endpoint 即使已经注册到 service registry，也只能由控制面 / 运维面访问；写操作仍要经过认证、授权、审计和明确目标实例策略
- `game-server.internal`、`game-server.proxy-local`、`proxy-local` / local socket 等 internal/local endpoint 只能在同机、内网或指定工作负载范围内可达；socket 路径、service token 和 `GAME_INTERNAL_TOKEN` 不能泄露到客户端或公网配置
...
- `public` / `internal` / `admin` / `local` visibility 不能替代防火墙、安全组、网关、TLS、token/RBAC 或 IP allowlist；registry endpoint 只能作为发现数据，不能作为暴露许可
- `test`、`staging`、`prod`、`production` 或 `DISCOVERY_REQUIRED=true` 下，旧 host/port/socket fallback 必须禁用或 fail-fast；固定地址不能绕过 registry，也不能作为安全边界说明
...
说明：`AUTH_REQUIRE_TLS` 已由 `auth-http` 读取，开发/测试默认 `false`，production 默认 `true`；其余公共配置仍属于设计建议。
...
生产部署应将 `GAME_ADMIN_AUDIT_PATH` 覆盖为 `/var/log/myserver/admin-audit.jsonl`，并使用独立的 `game-server-audit` volume；上述示例中的相对路径仅表示应用当前默认值。该 JSONL 不由 Vector `docker_logs` source 合并到普通运行日志。
...
`admin-api` 第一阶段权限矩阵仍复用 `admin_accounts.role`，不新增数据库 schema：`viewer` 拥有审计、安全日志、玩家、维护状态和监控只读权限；`operator` 在只读权限外可调整玩家非封禁状态，并执行 GM 广播、发道具和踢人；`admin` 与兼容角色 `super_admin` 拥有全部权限，包括玩家/GM 封禁、维护模式写入、监控归档和管理员 token 生命周期操作。权限拒绝统一返回 `403 INSUFFICIENT_PERMISSION` 并写结构化日志，但不向响应泄露完整权限矩阵。

该矩阵是当前代码基线，不是后续控制面最终模型。管理权限与高风险操作将以一次性切换方式替换为数据库化 permission/scope、`request_id` 幂等、预检 nonce、追加审计和短时签名内部操作断言；不会保留旧 role、通用 admin token 或旧写接口的运行时 fallback。development/test 使用独立 bootstrap 身份和密钥，但不允许关闭授权。详见[管理权限与高风险操作治理设计](../后台与运维/管理权限与高风险操作治理设计.md)。
...
3. 管理面、Redis、PostgreSQL、admin 端口默认不暴露公网；`game-proxy` admin HTTP 口已有 token 鉴权和生产默认 token 拒绝，仍需部署侧网络隔离
...
- service registry 中的 endpoint visibility 不被当作加密、鉴权或公网暴露许可；公网、内网和管理面仍分别通过 TLS、网关、防火墙 / security group、token / RBAC 和 IP allowlist 控制
...
- 后续一次性切换后，高风险操作使用预检、短时 nonce、`request_id` 和签名操作断言；旧 token/actor 字符串不再作为下游状态变更凭据，详见[管理权限与高风险操作治理设计](../后台与运维/管理权限与高风险操作治理设计.md)

### 11.4 防火墙 / 黑白名单

- Redis、PostgreSQL、管理端口默认不暴露到公网
- `game-server.admin`、`game-proxy.admin`、`admin-api` 等 admin endpoint 注册到 registry 后仍只允许控制面 / 运维面访问，不能从公网直连
- `internal` / `local` endpoint 只允许内网、同机或指定工作负载访问；local socket 路径和内部 token 不出现在客户端配置或公开日志中
...
- 黑白名单和封禁策略可跨实例同步
- 测试、预发和生产准入验证 `REGISTRY_ENABLED=true`、`DISCOVERY_REQUIRED=true`，并确认旧直连 fallback 未绕过 service registry
- NATS、Redis `chat:online`、proxy route store、`metrics:heartbeat:*` 不被用作 service discovery、认证授权或 endpoint 暴露依据
...
1. `docs/安全与监控/限流与安全现状.md`
2. `docs/后台与运维/管理后台设计.md`
