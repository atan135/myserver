...
生产公网暴露总口径以 [生产拓扑与 Room 迁移设计](../后台与运维/生产拓扑与Room迁移设计.md) 为准：正式玩家客户端只应依赖 `auth-http` 和 `game-proxy`，其它业务服务默认内网化。内部服务固定 host/port 只允许作为本地开发或手工调试 fallback；测试、预发和线上必须通过部署入口、registry endpoint 或 instance id 解析目标，不能把直连内部服务作为准入路径。
