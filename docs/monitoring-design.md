...
- 归档前会按 `service + bucket` 聚合多个实例 key；当前 MySQL 表仍保存服务级汇总，`extra` 仅记录 `instance_ids` / `instance_count` 等聚合元数据，不保存每个实例的完整历史曲线
...
7. Redis 实时 metrics 已支持多实例 key 和实例明细，但 MySQL 归档表尚未支持按实例维度回查历史。
...
4. 如果后续补上 MySQL 历史查询、服务注册联动或更细粒度权限控制，本文需要继续同步。
