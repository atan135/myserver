...
| `game-server admin` | `7500` | Rust + Tokio | 内部管理口；测试/线上消费者通过 service registry 发现 endpoint，本地默认端口仅作手工 fallback |
| `game-proxy` | `4000` | Rust + Tokio KCP / TCP fallback | 客户端游戏入口，校验 ticket 与维护模式后转发到 `game-server` 本地 socket；本地调试保留 TCP fallback |
| `game-proxy admin` | `7101` | Rust + Tokio | 内网控制面；测试/线上通过 service registry 或部署控制面发现 endpoint，本地默认端口仅作手工 fallback |
| `chat-server` | `9001` | Rust + Tokio | 单聊、群聊、聊天历史、邮件通知推送；测试/线上 endpoint 通过登录响应或注册中心发现 |
| `match-service` | `9002` | Rust + tonic gRPC | 匹配池、撮合、向 `game-server` 发起房间协作；测试/线上 endpoint 通过注册中心发现 |
| `announce-service` | `9004` | Node.js + NestJS + Fastify | 公告 CRUD、read token / game ticket 保护的有效公告查询、服务注册与监控上报；测试/线上 endpoint 通过登录响应或注册中心发现 |
| `mail-service` | `9003` | Node.js + NestJS + Fastify | 邮件 CRUD、邮件通知发布、部分服务注册接入；测试/线上 endpoint 通过登录响应或注册中心发现 |
...
- 统一的 `services` 对象；测试/线上客户端应优先消费该对象中的 endpoint，其中 `chat` / `mail` / `announce` 可由注册中心动态发现
...
当 `REGISTRY_ENABLED=true` 时，`match-service` 会优先从注册中心中读取 `game-server` 的 `metadata.internal_socket`；本地开发在关闭注册中心或 metadata 缺失时可回退到 `GAME_SERVER_INTERNAL_SOCKET_NAME`，测试/线上应修复 registry 数据，不依赖该静态配置。
...
- 服务发现尚未完全统一到一套实现上
- `auth-http` 登录响应当前会返回统一的 `services` 对象，同时保留 `gameProxyHost/gameProxyPort` 兼容字段；测试/线上消费者应以 `services` 为准
...
- `game-proxy` -> `4000`
- `game-server` -> `7000`
- `game-server admin` -> `7500`，仅表示本地默认内部管理口；测试/线上消费者通过 service registry 发现 endpoint
...
- `game-proxy admin` 当前仍使用代码默认值 `7101`，属于代理自身的内部管理口，不在 `apps/port.txt` 的固定入口清单里；测试/线上消费者应通过 service registry 或部署控制面发现 endpoint
- `chat-server`、`match-service`、`announce-service`、`mail-service` 属于内部服务；文档中的 `9001/9002/9004/9003` 仅用于本地开发与默认示例，测试/线上消费者不得硬编码这些地址，应以登录响应或注册中心信息为准
