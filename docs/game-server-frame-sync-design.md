...
- 服务边界以 [architecture.md](./architecture.md) 为准。
- 消息号、包头和 proto 字段以 [protocol.md](./protocol.md) 与 `packages/proto/game.proto` 为准。
- 更新、灰度、迁移口径以 [game-server-update-strategy.md](./game-server-update-strategy.md) 和 [game-server-room-rollout-spec.md](./game-server-room-rollout-spec.md) 为准。
...
1. 非 `InGame` 房间不推进帧。
2. 没有在线玩家输入参与者时暂停推进并保留等待状态。
3. 当前等待帧为 `current_frame + 1`。
4. 按策略判断是否可以推进：
   - `Strict`：所有在线玩家输入都到齐，或等待超时。
...
跨服迁移和空房接管灰度不在本文展开，详见 [空房接管式灰度规范](./game-server-room-rollout-spec.md) 和 [空房接管式灰度任务清单](./game-server-room-rollout-task-list.md)。
