...
- [x] 新增受控 debug/GM 调试协议，用于授予/撤销称号、设置职业阶位、触发称号解锁检查，必须要求独立 P2 debug token。（验证：`packages/proto/game.proto:750` 定义 debug 请求，`apps/game-server/src/core/service/character_title_service.rs:230` 校验 token，服务端只读取 `MYSERVER_CHARACTER_TITLE_DEBUG_TOKEN`，不回退到通用管理 token）
...
- [x] 管理后台查询接口必须进行管理员鉴权和审计，不能复用玩家 debug token。（验证：`apps/admin-api/src/players/players.controller.ts:25` 使用 `JwtAuthGuard`/`AdminPolicyGuard`，`apps/admin-api/src/players/players.controller.ts:109` 使用 `@Permissions("players.read")`，`apps/admin-api/src/players/players.controller.ts:131`/`150` 写成功/失败审计，`rg` 未发现该接口读取玩家 debug token）
...
- 验证记录：主 agent 复跑 `node --test --experimental-test-isolation=none --test-concurrency=1 tests/characters/db-init-characters.test.mjs tests/mock-client/mock-client-protocol.test.mjs`，24 passed；复跑 `cargo test title_table_ --manifest-path apps/game-server/Cargo.toml -- --nocapture`，7 passed；复跑 `cargo test character_ --manifest-path apps/game-server/Cargo.toml`，32 passed；复跑 `cargo test character_title_unlock --manifest-path apps/game-server/Cargo.toml`，7 passed；复跑 `cargo test character_title_service --manifest-path apps/game-server/Cargo.toml`，2 passed；复跑 mock-client `node --check` 通过；复跑 `git diff --check` 通过，仅 Git CRLF 提示。用户确认后已执行真实联调，见下方“真实联调记录”；`npm run check:mock-client-protocol` 仍受外部 `C:\project\mybevy` 缺少 `protocol.rs` 影响，未作为阶段 6 必过项。

- [x] 增加数据库结构测试，覆盖 `character_disciplines`、`character_titles`、`character_title_logs` 字段、约束和索引。（验证：`tests/characters/db-init-characters.test.mjs:172`/`200` 覆盖职业表字段/索引，`tests/characters/db-init-characters.test.mjs:220`/`254` 覆盖称号表字段/索引，`tests/characters/db-init-characters.test.mjs:273`/`304` 覆盖日志字段/索引，node 测试 24 passed）
...
- 开始时间：2026-06-26 17:42:53 +08:00
- 结束时间：2026-06-26 18:26:34 +08:00
- 服务依赖：PostgreSQL `5432`、Redis `6379`、Core NATS `4222`、auth-http `3000`、admin-api `3001`、game-server `7000/7500`、game-proxy `4000/7101/14000` 已启动并监听。
- 数据库初始化：对本地 `myserver_game` 执行 `db/init.sql`，补齐 `character_disciplines`、`character_titles`、`character_title_logs` 等 P2 表结构。
- 发现与修复：真实后台查询首次返回 500，日志显示 `character_titles` 不存在；原因是 `admin-api` 使用主 `DATABASE_URL` 指向 `myserver_auth` 查询 P2 游戏表。已增加 `GAME_DATABASE_URL` / `ADMIN_GAME_DATABASE_URL`、独立 `ADMIN_GAME_DB_POOL` 和 `createGameDbPool()`，使 P2 角色称号只读查询走 `myserver_game`，审计和后台账号仍走主库。
- 验证记录：`npm test --workspace admin-api` 通过，109 passed；`character-titles-debug` 使用本地账号和 `--title-id 1001` 返回 `ok: true`，装备称号 `1001` / `初心者`；`character-disciplines-debug` 使用 `forging/novice/1` 返回 `ok: true`，职业阶位 active；错误 debug token 场景返回 `CHARACTER_TITLE_DEBUG_FORBIDDEN`；`GET /api/v1/players/characters/chr_1rry968g0020/titles?logLimit=3` 返回 HTTP 200，包含 3 个称号、装备称号 `1001`、职业阶位 `forging/novice` 和称号日志。
- 备注：`--title-id 9001` 会授予隐藏称号 `系统观察员`，但装备被 `INVALID_TITLE_ACTION` 拒绝，这是隐藏称号不可主展示的预期行为；正向装备验收使用非隐藏称号 `1001`。
...
- 开始时间：2026-06-26 14:21:07 +08:00
- 结束时间：2026-06-26 18:26:34 +08:00
- 验收总结：P2 称号与职业阶位基础已完成代码、测试、正式文档和真实服务联调闭环；真实联调覆盖 mock-client 称号授予/装备、职业阶位设置、debug token 负例和 admin-api 只读查询，并修复了 admin-api P2 查询误连主库的问题。

- [x] 空库执行 `db/init.sql` 后包含 `character_disciplines`、`character_titles` 和 `character_title_logs`，并具备必要约束和索引。（验证：`db/init.sql:354`/`381`/`415` 建表，`tests/characters/db-init-characters.test.mjs:172`-`304` 静态测试覆盖字段/约束/索引）
