...
- [x] 更新 `docs/游戏服与接入层/角色与成长/角色体系与四属性设计.md`，将 P0 兼容边界改为已迁移完成或新阶段目标态。（验证：`docs/游戏服与接入层/角色与成长/角色体系与四属性设计.md` 顶部现状和 P0 边界说明房间、匹配、输入、移动、战斗、背包和 transfer 已按角色 ID 建模，账号 `playerId` 仅用于登录、安全、封禁、kick/revoke、ticket version 和 mail ownership）
