...
这份文档把 [空房接管式灰度发布技术规范](./空房接管式灰度规范.md) 拆成可执行任务清单。
...
- [x] 确认 [空房接管式灰度规范](./空房接管式灰度规范.md) 作为唯一规范源。
...
- 后续开发不再一边编码一边改术语。
- 当前状态：`空房接管式灰度规范.md` 已明确为唯一规范源；room 接管判定和第一阶段显式重连约束已冻结；协议名称、字段名、状态枚举和消息编号已经固定到 `packages/proto/game.proto`。
...
- `game-server` 已通过已鉴权 admin/internal 控制通道增加 `RequestServerShutdownReq/Res`，并由 `auth-http` 暴露为 `POST /api/v1/internal/game-server/shutdown-if-drained`。响应三态为：`ok=true, shutdown_armed=true` 先写回成功再立即触发 graceful shutdown；`ok=false, shutdown_armed=true` 返回 blocker 错误和计数，同时接受默认 `300s` 有界等待；`ok=false, shutdown_armed=false` 表示拒绝或未能武装。等待超时不会强杀会话或 room，而会解除武装，后续显式请求可重试；重复请求不重置当前期限。`retired_room_count` 只作为观测字段，不阻塞停服。`tools/mock-client` 已增加 `request-server-shutdown` 场景用于人工演练。
...
- `scripts/ops/rollout-three-process-drill.ps1` 已提供第一阶段 old/new/proxy 演练入口，默认 dry-run，只做工具检查、端口探测、步骤命令输出和 `rollout-transfer-cli --dry-run` JSON 计划校验；显式 `-ExecuteSteps` 才调用已运行服务的控制面接口，`request-server-shutdown` 还需要额外 `-AllowShutdownRequest`。详见 [old/new/proxy 三进程 rollout 演练入口](../后台与运维/三进程灰度演练手册.md)。2026-06-13 已在真实 old/new/proxy/auth 环境中人工执行空房迁移控制面并通过；该验收仍未沉淀为自动测试准入。
...
- [x] `complete-if-drained` 启用旧服校验时，满足旧服真实状态条件才允许结束 rollout:
  ...
  - `owned_room_count == 0`
  - `migrating_room_count == 0`
- [x] game-server 受控 graceful shutdown 入口使用同一组旧服状态决定立即退出或接受有界等待:
...
- [x] 在 game-server 自身提供显式武装的受控 graceful shutdown 请求入口，blocker 未清零时可进入默认 `300s` 有界等待，超时保护会话并允许重试。
...
- [x] 第一阶段 old/new/proxy 演练脚本入口已具备。
  - 当前实现说明：`scripts/ops/rollout-three-process-drill.ps1` 可按 preflight、rollout start、old drain、空房选择说明、room transfer 编排、drain status、complete-if-drained 和可选 shutdown safety gate 输出或执行步骤；默认 dry-run，不启动服务，不调用写接口，并会调用 `tools/rollout/rollout-transfer-cli.js --dry-run` 输出 room transfer JSON 计划。`rollout-transfer-cli --dry-run` 已有 Node test 覆盖计划结构、缺失参数和 same-server 拒绝。
...
- `tools/rollout/rollout-fault-drill-cli.js` 已提供脚本级故障演练入口。默认 `dry-run` 只输出 JSON 计划，不访问服务、不调用写接口、不请求旧服停服；`--simulate` 使用纯 mock client 验证编排停止点；只有显式 `--execute` 才调用已运行服务的控制面接口。详见 [rollout 故障演练入口](../后台与运维/灰度故障演练手册.md)。
