...
- 当前服务边界以 [整体架构](../总览/整体架构.md) 为准
- 当前协议号和字段以 [协议设计](../协议与客户端/协议设计.md) 为准
- 已落地的房间运行时细节以 [帧同步与房间生命周期设计](./帧同步与房间生命周期设计.md) 为准
...
- `game-server` 已有 admin 口、internal socket、metrics、PostgreSQL 审计、服务注册和 session kick
...
- PostgreSQL 账号、审计、连接事件、房间事件、玩家背包数据等持久化
- metrics 归档服务
...
- [空房接管式灰度规范](./空房接管式灰度规范.md)
- [CSV 配置表设计](../配置与场景/CSV配置表设计.md)
