...
- 正式 drain/shutdown 写入口为 admin-api 的 `POST /api/v1/rollouts/game-server/:instanceId/drain` 与 `POST /api/v1/rollouts/game-server/:instanceId/shutdown`；两者要求显式 instance ID、Redis registry 中健康 `game-server.admin` 精确发现、Bearer/JWT、high-risk preflight/execute、独立审批、签名断言和脱敏审计。drain 使用 `game.config.write`，由控制面绑定 `worldId=*`、`serviceName=game-server` 和显式 instance；shutdown 使用 emergency-only 的 `service.shutdown`，绑定 `serviceName=game-server` 和显式 instance，执行阶段还要求匹配目标的有效 break-glass grant。固定内部 endpoint 不进入该流程，`auth-http` 的历史写接口保持 `410 CONTROL_PLANE_ONLY`。
