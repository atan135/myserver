...
- [空房接管式灰度任务清单](./空房接管式灰度任务清单.md)
- 生产拓扑、多实例与同连接迁移目标态见 [生产拓扑与 Room 迁移设计](../后台与运维/生产拓扑与Room迁移设计.md)

规范源声明:

- `docs/游戏服与接入层/空房接管式灰度规范.md` 是“空房接管式灰度发布”的唯一规范源。
- `docs/游戏服与接入层/空房接管式灰度任务清单.md` 只是基于本规范的执行拆分，不得覆盖或改写本规范结论。
...
截至 `2026-08-08`, 本文仍包含“空房接管式灰度”的目标规范, 不代表所有能力已经落地。
...
- `game-proxy` 已有 `RolloutSession`、`RoomRouteRecord`、`CharacterRouteRecord`、room/character 路由选择和 admin 查询/更新接口。
...
- `game-server` 开启 drain 后发布 unhealthy，proxy 不再把它用于新的默认路由；玩家 TCP 与 `proxy-local` listener 仅为已有 room 的离线成员保留鉴权和 reconnect transport，`AuthReq` 拒绝不属于本实例离线 room 的新角色并返回 `SERVER_DRAINING_REJECT_NEW_SESSION`，业务层同时拒绝创建新 room、匹配建房等新分配，并保留已有连接、room 和 admin/internal 控制通道。
- `game-server` 已按 `SERVICE_INSTANCE_ID` 派生独立 `proxy-local` / `internal` socket，并在 worker lease 后执行只针对本实例路径的有界 stale reclaim；old/new 可使用不同实例 ID、worker ID 和 socket 并存。
- `game-proxy` 在测试、预发和线上只消费 registry 中 healthy 的 `proxy-local.socket` 精确路径，旧实例 drain 后不会继续成为新连接的健康默认 upstream。
...
- 正式 drain/shutdown 写入口为 admin-api 的 `POST /api/v1/rollouts/game-server/:instanceId/drain` 与 `POST /api/v1/rollouts/game-server/:instanceId/shutdown`；两者要求显式 instance ID、Bearer/JWT、high-risk preflight/execute、独立审批、签名断言和脱敏审计。drain 使用常规 healthy registry discovery 和 `game.config.write`，由控制面绑定 `worldId=*`、`serviceName=game-server` 和显式 instance。drain 后实例会按设计发布 unhealthy，不再进入玩家路由、readiness 或其他 GM 写操作；仅 emergency-only `service.shutdown` 路径可显式启用隔离的 live admin discovery，以相同 instance ID 查找 heartbeat/TTL 仍活跃、schema/identity 合法且 endpoint 声明 `name=admin`、`visibility=admin`、`protocol=tcp` 的控制口。endpoint 的 `healthy` 是实例整体 readiness 的投影，drain 后允许为 false；shutdown 仍须实际连接该 endpoint、通过签名断言认证并收到停机响应，连接、认证或响应失败均 fail-closed。shutdown 执行阶段还要求匹配目标的有效 break-glass grant；不允许 fallback、direct endpoint、过期 heartbeat 或错误的 admin 传输元数据。固定内部 endpoint 不进入该流程，`auth-http` 的历史写接口保持 `410 CONTROL_PLANE_ONLY`。
...
- 旧服进程已具备部署平台 stop hook：经校验的 release bundle 会在共享锁内用 durable journal 事务化安装完整运维脚本集合与 apply runner；候选 Ready 后，`ops-retire.sh` 以确认串中的 Compose project、实例 ID 和完整镜像 revision 围栏旧容器，临时关闭其自动重启并等待 admin-api 正式审批链触发 graceful shutdown。失败恢复原 desired state，异常 journal 必须显式恢复；recover 会重启旧实例并结束当前 retire，后续必须重新发起 retire 周期。
- 部署平台自动创建 old/new 实例、注入唯一 instance/worker identity 并批量编排候选创建、切流和 stop hook；当前 helper 本身不创建 candidate。

因此阅读本文时应把它当作后续实现约束；当前代码状态以 [空房接管式灰度任务清单](./空房接管式灰度任务清单.md) 和实际代码为准。
...
- 维护 room 级路由元数据
- 按 `room_id` / `character_id` 进行接入决策
- 区分旧房角色和新房角色应该进入哪台 server
- 在旧服彻底排空后结束灰度模式
...
当前 checksum 规则：导出端清空 `checksum` 字段后，对 `RoomTransferPayload` 的 protobuf 编码计算 SHA-256 hex；导入端按同样规则复算并比较。为减少非确定性，`RoomSnapshot.members` 在生成时按 `character_id` 排序。
