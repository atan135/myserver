...
- 正式 drain/shutdown 写入口为 admin-api 的 `POST /api/v1/rollouts/game-server/:instanceId/drain` 与 `POST /api/v1/rollouts/game-server/:instanceId/shutdown`；两者要求显式 instance ID、Bearer/JWT、high-risk preflight/execute、独立审批、签名断言和脱敏审计。drain 使用常规 healthy registry discovery 和 `game.config.write`，由控制面绑定 `worldId=*`、`serviceName=game-server` 和显式 instance。drain 后实例会按设计发布 unhealthy，不再进入玩家路由、readiness 或其他 GM 写操作；仅 emergency-only `service.shutdown` 路径可显式启用隔离的 live admin discovery，以相同 instance ID 查找 heartbeat/TTL 仍活跃、schema/identity 合法且 endpoint 自身仍为 `healthy=true`、`visibility=admin`、`protocol=tcp` 的 `game-server.admin` 控制口。shutdown 执行阶段还要求匹配目标的有效 break-glass grant；不允许 fallback、direct endpoint、过期 heartbeat 或不健康 admin endpoint。固定内部 endpoint 不进入该流程，`auth-http` 的历史写接口保持 `410 CONTROL_PLANE_ONLY`。
