...
- 客户端 KCP 入口，本地默认 `PROXY_PORT=4000`；固定端口只表示本地监听默认值，不是测试、预发、线上直连配置。
- TCP fallback 入口，本地默认 `PROXY_TCP_FALLBACK_PORT=PROXY_PORT+10000`，用于本地调试和兼容。
- 上游 `game-server` 使用 local socket 连接；测试、预发、线上主路径必须通过 service registry 发现 `game-server.proxy-local` endpoint，endpoint 模型为 `name=proxy-local`、`protocol=local_socket`、`visibility=local`。
- `UPSTREAM_SERVER_ID`、`UPSTREAM_LOCAL_SOCKET_NAME` 仅作为 `development` / `local` 且 discovery 非 required 时的本地静态 upstream fallback；strict、test、staging、production 必须 `REGISTRY_ENABLED=true`、`DISCOVERY_REQUIRED=true`，不能依赖这些 legacy 变量。
- Redis service registry 发现：`REGISTRY_ENABLED`、`REGISTRY_URL`、`UPSTREAM_SERVICE_NAME`；当前 upstream 消费 `proxy-local` endpoint，不把 legacy `local_socket` 字段、`game-server.client` 或 `game-server.internal` 当作当前主路径。
- `game-proxy` 自身在启用 registry 时注册 `client`、`client-tcp-fallback`、`admin` endpoint；`admin-api` 可通过 `game-proxy.admin` 聚合多 proxy 的监控 / rollout 读路径。
...
- rollout session、room route、character route 的 route store；默认内存态，本地开发无需 Redis，生产可通过 `PROXY_ROUTE_STORE_BACKEND=redis` 持久化到 Redis。
- 根据 `RoomJoinReq`、`RoomJoinAsObserverReq`、`RoomReconnectReq` 做最小协议感知路由。
- 成功加入 / 重连 / 观战后绑定 room owner 和 character route。
- room route 的版本、epoch、checksum 校验。

已推进到第一阶段闭环：

- Redis route store 持久化已形成第一阶段多 proxy 闭环：启用后启动加载已有 rollout session、room route、character route，变更后以单 key 快照级 CAS 写回 Redis，并在 CAS 成功后通过 Redis pub/sub 发布最新 `store_revision`；其它 proxy 收到比本地更新的 revision 后会 reload Redis 快照，解决单 proxy 重启丢 route，并缩短多 proxy 本地缓存陈旧窗口。它是第一阶段共享 route store，不是完整强一致控制面；写操作仍需要控制面 owner / 目标 proxy 策略，不能在多个 proxy 上无约束并发写。
- `game-proxy` 已支持基于当前 rollout route store 的自动收尾：控制面可检查当前 epoch 内是否仍有 old owner / 迁移中 room route 或指向 old 的 character route，排空后自动结束 rollout 并清理当前 epoch 和空 epoch route metadata。
...
- `game-server` 已通过已鉴权 admin/internal 通道提供 `RequestServerShutdownReq/Res` 受控 graceful shutdown 入口，并由 `auth-http` 暴露为 `POST /api/v1/internal/game-server/shutdown-if-drained`。响应三态为：`ok=true, shutdown_armed=true` 立即 graceful shutdown；`ok=false, shutdown_armed=true` 表示连接或 room blocker 仍在但已接受默认 `300s` 有界等待；`ok=false, shutdown_armed=false` 表示拒绝或未能武装。等待超时不会强杀现有会话或 room，而会解除武装并允许后续显式请求重试；`retired_room_count` 只作为观测字段。`tools/mock-client` 可用 `request-server-shutdown` 场景人工演练该入口。
...
- ticket 的最小接入鉴权。
- 维护模式下拒绝新 `AuthReq` 接入。
- 根据默认 upstream、room route 或 character route 选择 `game-server`。
...
mybevy client / mock-client
  ...
  -> game-proxy(KCP, TCP fallback)
    -> game-server(proxy-local local socket)
```

普通流程：

1. 客户端从 `auth-http` 获得 ticket 和 proxy 地址；测试、预发、线上该地址来自 `game-proxy.client` 发现结果，本地非严格环境可使用兼容 fallback。
2. 客户端连接 `game-proxy`。
3. 客户端发送 `AuthReq`。
...
9. `game-proxy` 根据请求类型选择 upstream；测试、预发、线上 upstream 来自 registry 中的 `game-server.proxy-local` endpoint。
10. `game-proxy` 建立到 `game-server` 的 `proxy-local` local socket 连接。
...
- registry discovery：测试、预发、线上当前主路径。`game-proxy` 启动时刷新 `UPSTREAM_SERVICE_NAME`（默认 `game-server`）的实例列表，消费健康且合法的 `proxy-local` endpoint；当前解析要求 endpoint `protocol=local_socket` 且 `socket` 非空。registry 发布和准入模型应为 `name=proxy-local`、`protocol=local_socket`、`visibility=local`。route 的 `server_id` 优先取 endpoint metadata 中的 `instance_id` / `server_id`，否则使用 registry instance id；`local_socket_name` 来自 endpoint 的 `socket` 字段。
- local static fallback：仅 `REGISTRY_ENABLED=false`、discovery 非 required，且 `NODE_ENV=development` 或 `APP_ENV=local` 时允许。此时用 `UPSTREAM_SERVER_ID` + `UPSTREAM_LOCAL_SOCKET_NAME` 填充静态 route store，用于单机本地调试。

当前不会把 `ServiceInstance.local_socket` 兼容字段、endpoint name 为 `local_socket` 的 legacy endpoint、`game-server.client` 或 `game-server.internal` 当作 `game-proxy` upstream 主路径。registry enabled 时初始发现不到任何 `game-server.proxy-local` 会 fail-fast；后续 watch 发现错误或空列表时，会把已知 route 同步为 `Unavailable`，新连接无法选到可用 upstream，而不是回退到静态 upstream 或其它 endpoint。
...
### 6.4 CharacterRouteRecord

当前 character route 包含：

```text
CharacterRouteRecord {
  character_id,
  current_room_id,
  preferred_server_id,
  rollout_epoch,
...
`RoomReconnectReq` 是空 body；proxy 使用已鉴权连接的 `character_id` 查询 character route，并结合 room route 选择 upstream。

### 6.5 Route Store 持久化

`ProxyRouteStore` 当前支持两种 backend：

- `memory`：默认值，所有 rollout session、room route、character route 仅保存在 proxy 进程内，适合本地开发和不启 Redis 的单机调试。
- `redis`：启动时从 Redis 加载已有 route store 快照，后续在 begin/end rollout、rollout state 更新、room route/character route upsert、成功 join/reconnect/observer 后的 `bind_room_owner` 中以乐观并发控制写回快照。
...
当前持久化解决的是单 proxy 重启恢复 rollout session、room route、character route 的最低风险，并提供单 key 快照级 CAS 和 pub/sub reload 来避免直接最后写覆盖、缩短跨 proxy 缓存失效窗口。它不持久化 upstream health/operation state，也不是完整强一致方案；Redis CAS 能拒绝陈旧 revision 写入，但不能替代控制面 owner、目标 proxy 策略、分布式锁或冲突合并。多 proxy 部署下，读路径可以通过各 proxy 的 Redis reload 快照逐步收敛；写路径必须由控制面明确写入 owner 或目标实例，避免多个 proxy 对 rollout / route metadata 做无约束并发写。
...
| `RoomReconnectReq` | 空 body；根据已鉴权 `character_id` 对应的 character route / room route 路由 |
| `RoomJoinRes` / `RoomJoinAsObserverRes` / `RoomReconnectRes` | 成功后绑定 room owner 和 character route |
...
当前 `game-proxy` admin 口是轻量 HTTP，本地默认监听 `PROXY_ADMIN_HOST:PROXY_ADMIN_PORT`。启用 registry 时，`game-proxy` 会发布 `admin` / `http` / `admin` endpoint；测试、预发、线上控制面应通过 `game-proxy.admin` 发现，不应硬编码本地默认端口直连。所有 admin 请求都需要 admin token 鉴权；`PROXY_ADMIN_TOKEN` 是全权限写 token，兼容所有 GET/POST；可选 `PROXY_ADMIN_READ_TOKEN` 是只读 token，仅允许 GET；`PROXY_ADMIN_SCOPED_TOKENS` 可配置额外受限 token。当前兼容两种 header 形式：
...
| 权限 | 允许操作 |
|------|----------|
| `proxy.read` | `GET /status`、`/instances`、`/rollout`、`/room-routes`、`/character-routes` |
...
| `proxy.route.write` | `POST /room-route/upsert`、`/character-route/upsert`、`/switch/<server_id>` |
...
| `GET` | `/character-routes` | character route 列表 |
...
| `POST` | `/rollout/end` | 在 route store 已排空时结束 rollout 并清理相关 route；仍有 old owner / 迁移中 room route 或指向 old 的 character route 时返回 `409 ROLLOUT_NOT_DRAINED` |
| `POST` | `/rollout/state?state=Active|Ending|Interrupted` | 更新 rollout 状态 |
| `POST` | `/rollout/complete-if-drained` | 自动收尾：若当前 epoch 内仍有 old owner / 迁移中 room route 或指向 old 的 character route，返回 `409` 和阻塞计数/示例；若已排空则结束 rollout 并返回清理摘要 |
| `POST` | `/room-route/upsert?...` | 手动 upsert room route；校验必填字段、迁移状态枚举、成员数、版本号、checksum 长度和 upstream 存在性 |
| `POST` | `/character-route/upsert?...` | 手动 upsert character route；校验 character/room/server id、upstream 存在性和 rollout epoch |
| `POST` | `/switch/<server_id>` | 将目标 upstream 置为 active，其余置为 draining |

当前 admin 修改接口会记录结构化日志审计，包含 `action`、操作人、关键目标（`server_id` / `room_id` / `character_id` / `rollout_epoch`）和 `result=ok|error`，不会记录 token。权限不足返回 `403 insufficient admin permission`；写操作权限拒绝会尽量写入 JSONL 审计，`error=insufficient_permission`，审计写入失败时至少记录 structured warn。启用 Redis route store 时，admin 写入会同步更新 route store 快照；持久审计第一阶段采用本地 JSONL 文件，尚未接入 PostgreSQL 审计查询或集中留存。

多 proxy 场景下，`admin-api` 读路径可以发现多个 `game-proxy.admin` endpoint 并聚合 `/status`、`/rollout` 等只读结果。写路径不能简单向多个 proxy 并发广播：当前 Redis backend 只提供单 key revision/CAS 和 pub/sub reload 的第一阶段共享 route store，仍需要控制面明确 owner / target proxy / rollout 操作策略。对 rollout、route upsert、switch、maintenance 等写操作，应避免多个 proxy 无约束并发写同一份 route metadata。
...
- `game-proxy`：上游状态、rollout session、room/character route。
- `game-server`：server 级 `drain_mode` 除阻止创建新房外，还停止玩家 TCP 和 `proxy-local` 的新 accept 并发布 unhealthy；开启 drain 前已经建立的连接仍可对已有房执行 join/reconnect/observer，admin/internal 控制通道继续可用。
...
- `game-server` drain 开启后不再接受新的玩家 TCP / `proxy-local` 连接，已有连接任务不会因 drain 被 abort。
- old/new `game-server` 通过不同 `SERVICE_INSTANCE_ID` 派生独立 socket；proxy discovery 使用 healthy `proxy-local` endpoint 的精确 `socket` 字段，因此新实例 Ready 后可加入 route，旧实例 drain/unhealthy 后从新连接候选中移除。
- `game-proxy` 可按 room route / character route 将相关请求送回旧 owner 或送到新 owner。
- proxy 已能基于当前 rollout route store 判断是否可自动结束 rollout。当前判断范围是 proxy 已知的 room route / character route：当前 epoch 内 `owner_server_id == old_server_id`、迁移状态仍在 old/transfer 中，或 character route 仍指向 old server 时会阻止结束；已切到 `new_server_id` 的 route 不阻止结束。带其它 epoch 的陈旧 route 不阻止当前 rollout 自动收尾，结束时只清理当前 epoch 和空 epoch route；这些陈旧记录仍需要后续 TTL/巡检或控制面清理策略处理。
...
| 变量 | 说明 | 默认 |
|------|------|------|
| `PROXY_HOST` | KCP 监听 host；本地默认监听示例，不是测试/线上服务间直连配置 | `127.0.0.1` |
| `PROXY_PORT` | KCP 监听端口；测试/预发/线上客户端入口应来自 `game-proxy.client` 或部署入口 | `4000` |
| `PROXY_ADMIN_HOST` | admin 监听 host；本地默认监听示例 | 同 `PROXY_HOST` |
| `PROXY_ADMIN_PORT` | admin 监听端口；测试/预发/线上控制面应发现 `game-proxy.admin` | `7101` |
...
| `PROXY_TCP_FALLBACK_HOST` | TCP fallback host；仅用于本地调试和兼容 | 同 `PROXY_HOST` |
| `PROXY_TCP_FALLBACK_PORT` | TCP fallback 端口；仅用于本地调试和兼容 | `PROXY_PORT + 10000` |
| `UPSTREAM_SERVER_ID` | 本地静态 upstream fallback 的 server id；只允许 `development` / `local` 且 discovery 非 required | `game-server-1` |
| `UPSTREAM_LOCAL_SOCKET_NAME` | 本地静态 upstream fallback 的 local socket；strict/test/staging/production 不允许依赖 | `myserver-game-server.sock` |
| `REGISTRY_ENABLED` | 是否启用服务发现；strict/test/staging/production 必须为 `true` | `false` |
...
| `DISCOVERY_REQUIRED` | 是否要求严格服务发现；为 `true` 或 `NODE_ENV` / `APP_ENV` 属于 strict/test/staging/production 时必须通过 registry 发现 `game-server.proxy-local` | `false` |
| `UPSTREAM_SERVICE_NAME` | 要发现的 upstream 服务名；当前消费该服务下的 `proxy-local` endpoint（`protocol=local_socket`、`visibility=local`） | `game-server` |
...
- `game-proxy` upstream 发现的当前主路径是 `game-server.proxy-local`；registry 发布和准入模型要求 endpoint `protocol=local_socket`、`visibility=local`。不要把 legacy `local_socket` 字段、`game-server.client` 或 `game-server.internal` 作为 proxy upstream 主路径。
- `UPSTREAM_SERVER_ID` / `UPSTREAM_LOCAL_SOCKET_NAME` 只保留给本地静态 fallback。strict、test、staging、production 中如果启用 required discovery，配置加载会拒绝 legacy 直连变量；初始发现不到 `game-server.proxy-local` 时应 fail-fast 或让 proxy 不可接流量。
- 固定端口、固定 host 和本地 socket 名只作为本地默认监听或 fallback 示例；测试、预发、线上应通过 service registry endpoint 或部署入口解析。
