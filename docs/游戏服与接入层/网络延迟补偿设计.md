...
1. **帧同步输入延迟处理**：解决 lockstep 帧同步中各角色输入到达时间不一致的问题
...
- `pending_inputs: HashMap<character_id, PlayerInput>` - 本帧已收到的输入
- `future_inputs: HashMap<frame_id, HashMap<character_id, PlayerInput>>` - 提前到达的输入
- `wait_deadline: Instant` - 本帧截止时间

**等待策略A：严格帧同步（等待所有角色）**
```
每帧 tick 时：
  if pending_inputs.count() == all_characters.count():
     // 所有角色输入都到了，立即推进
...
| **服务端职责** | 等待所有角色输入，按帧推进 | 定期广播权威状态 | 回溯到开枪时刻判定 |
