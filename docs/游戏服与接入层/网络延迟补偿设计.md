...
截至 `2026-05-19`, 本文是算法设计与后续开发参考, 不是所有算法的当前实现说明。

当前已经落地:

- 帧同步输入延迟处理已接入 `RoomRuntimePolicy.input_delay_frames`。
- 当前帧等待已支持 `Strict` 和 `Optimistic` 两种策略。
- 缺帧处理已支持 `Empty`、`RepeatLast`、`DropAfterMisses`。
- 服务端会拒绝过期输入和过远未来帧。
- `movement_demo` 已使用 `MovementSnapshotPush` / `MovementRejectPush` 做权威位移校正。
- 位移校正已接入周期、误差阈值和 AOI 半径配置。
- 断线重连和观战恢复可返回 `movement_recovery`。

当前尚未落地:

- 完整状态同步模式下的通用回滚重演框架。
- 面向所有玩法的增量状态广播框架。
- 射击命中判决回溯、历史 hitbox 和开枪时间校验。

因此当前项目的主链路仍是“输入帧同步 + 低频权威校正”, 不应理解为已经全面切到状态同步或 FPS/TPS 回溯模型。
...
| 优先级 | 类型 | 说明 |
|--------|------|------|
| 高 | 帧同步输入延迟处理 | 已有当前实现，后续重点是客户端配合、压测和按玩法调参 |
| 中 | 状态同步权威矫正 | `movement_demo` 已有位移校正样例，通用状态同步仍需评估 |
| 低 | 射击命中判决回溯 | 当前未实现，仅 FPS/TPS 类型游戏需要 |
