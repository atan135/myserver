...
- [x] mock-client 可以完成称号查询、测试授予、装备和结果验证流程。（验证：`tools/mock-client/src/scenarios/character.js:430` 实现 `character-titles-debug`，`tools/mock-client/README.md:293` 给出命令，`tests/mock-client-protocol.test.mjs:414` 覆盖异步匹配和 JSON 输出形态；真实联调 `character-titles-debug --title-id 1001` 返回 `ok: true` 且装备 `初心者`）
