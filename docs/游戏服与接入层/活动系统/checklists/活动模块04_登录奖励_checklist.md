...
- [x] 登录奖励属于运营活动，不复用任务/成就 progress_id。（验证：登录奖励状态存储使用独立的 PlayerActivityState/activity_claim 语义键，未复用任务或成就进度表。）
- [x] 登录事件由服务端产生，客户端不能提交登录天数。（验证：`on_game_entry` 从可信服务端 game_entry 事件推进状态，客户端协议不接受登录天数或进度字段。）
- [x] 日期按活动时区计算，period_key 和领取语义键由服务端生成。（验证：按活动 IANA 时区生成自然日 period_key，领取语义键由角色/活动/版本/阶段和周期服务端生成；时区/DST 测试通过。）
- [x] 阶段奖励通过统一领奖和资产交付入口结算。（验证：阶段领取统一经过 ActivityClaimCoordinator 和 RewardDeliveryService，交付失败保留可重试状态，不直接修改资产。）

## 引用文档

- [活动功能总纲](../活动功能总纲.md)：登录奖励的运营活动定位、时间、周期、阶段和领取规则。
- [任务系统设计](../../任务与世界/任务系统设计.md)：任务/进度模块边界；登录奖励不得复用任务 progress_id。
- [游戏业务模块开发规范](../../游戏业务模块开发规范.md)：活动来源与角色业务模块的职责边界。
- [统一资产事务与奖励交付运行边界](../../背包与物品/统一资产事务与奖励交付运行边界.md)：活动奖励交付、背包满 fallback、幂等和流水。
- [协议设计](../../../协议与客户端/协议设计.md)：活动详情、进度和领奖协议接入约束。
- [外部客户端接入说明](../../../协议与客户端/外部客户端接入说明.md)：客户端展示进度和领取结果的接入边界。
