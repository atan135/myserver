...
- [活动功能总纲](../../docs/游戏服与接入层/活动系统/活动功能总纲.md)：活动领域模型、版本、生命周期、类型注册和数据边界。
- [游戏业务模块开发规范](../../docs/游戏服与接入层/游戏业务模块开发规范.md)：业务模块目录、领域分层和跨模块职责。
- [数据库初始化说明](../../docs/数据库/数据库初始化说明.md)：数据库初始化和 schema 维护边界。
- [数据库迁移体系设计](../../docs/数据库/数据库迁移体系设计.md)：数据库迁移、兼容和回滚要求。
- [统一资产事务与奖励交付运行边界](../../docs/游戏服与接入层/背包与物品/统一资产事务与奖励交付运行边界.md)：奖励来源、资产流水、幂等和恢复边界。
- [服务注册中心设计](../../docs/周边服务/服务注册中心设计.md)：多实例服务发现和跨服务访问约束。
