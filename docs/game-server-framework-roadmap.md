...
- 发送队列已切换为有界 `mpsc::Sender`，广播路径使用 `try_send`；队列满会按 `outbound_queue_full` 关闭慢连接并进入断线清理，后续仍需补齐更细的指标和降级策略
- 玩家消息频率限制、非法包惩罚和更完整的风控边界仍需补齐
- `game-server` admin 口已支持状态、配置、drain、GM 发物品、广播、踢人和封禁在线连接处置；GM 广播、踢人和封禁已通过 NATS 完成跨实例在线连接处置，限时封禁由 `auth-http` 登录/签票路径惰性自动解封
...
- GM 广播、踢人、封禁已接入服务端 admin handler；GM 广播和 session kick 已通过 NATS 覆盖跨实例在线连接处置，后续仍需持续收敛失败原因码和审计查询
- 房间快照持久化接口
- 对局事件流、回放和可恢复状态版本
- 配置版本号、变更来源和灰度追踪
