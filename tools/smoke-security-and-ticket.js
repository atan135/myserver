#!/usr/bin/env node
/**
 * 安全与 Ticket live smoke 验证脚本
 *
 * 用法:
 *   node tools/smoke-security-and-ticket.js [--base-url http://127.0.0.1:3000]
 *
 * 这是针对已启动 auth-http、Redis、PostgreSQL 和角色表的 live smoke，
 * 不是 node:test 单元测试；纯代码回归覆盖在 tests/auth/ 下。
 *
 * 测试内容:
 * 1. IP 限流
 * 2. 账号锁定
...
async function main() {
  console.log("========================================");
  console.log("  安全与 Ticket live smoke 验证");
  console.log("========================================");
  console.log(`\n目标地址: ${BASE_URL}`);
